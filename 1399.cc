...
class Town {
 private:
  ...
  inline void push(queue<int>& Q, int r, int c, uint s) {
    Q.push((r << 8) + c);
    step[r][c] = s;
  }
...
bool Town::go(int sr, int sc, int dr, int dc) const {
  if (dr < 0 || dr >= h || dc < 0 || dc >= w ||
      step[dr][dc] <= step[sr][sc] + 1)
    return false;
  int diff = terrain[sr][sc] - terrain[dr][dc];
  if (diff > 3 || diff < -1)
    return false;
  else
    return see(dr, dc, bsr, bsc) || see(dr, dc, bdr, bdc);
}
...
bool Town::visible(int x1, int y1, int x2, int y2, bool subx) const {
  ...
  int z1 = subx ? terrain[x1][y1] : terrain[y1][x1],
      z2 = subx ? terrain[x2][y2] : terrain[y2][x2];
  int dx = x2 - x1, dy = y2 - y1, dz = z2 - z1, y = 2 * dx * y1 + dy,
      z = (2 * z1 + 1) * dx + dz;
  int cx, p = (dy > 0) ? 1 : 0;
  for (cx = x1 + 1; cx <= x2; cx++) {
    ...
    if (subx) {
      if (z < terrain[cx - 1][ry] * 2 * dx || z < terrain[cx][ry2] * 2 * dx)
        return false;
    } else {
      if (z < terrain[ry][cx - 1] * 2 * dx || z < terrain[ry2][cx] * 2 * dx)
        return false;
    }
    z += 2 * dz;
    y += 2 * dy;
...
int main() {
  ...
  for (t = 0; t < T; t++) {
    ...
    if (path == INF)
      ...
      printf("The shortest path is %u steps long.\n", path);
  }
