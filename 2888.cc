...
int phi(int x) {
  ...
  for (int i = 0; i < tot && pri[i] * pri[i] <= x; ++i) {
    if (x % pri[i] == 0) ret = ret / pri[i] * (pri[i] - 1);
...
struct Matrix {
  ...
  Matrix operator*(const Matrix &b) const {
    ...
    for (int i = 1; i <= m; ++i)
      for (int j = 1; j <= m; ++j) {
        for (int k = 1; k <= m; ++k) ret.g[i][j] += g[i][k] * b.g[k][j];
        ret.g[i][j] %= mod;
      }
    return ret;
  }
...
int gao(int x) {
  int res = 0;
  Matrix ret = qpow(a, x);
  for (int i = 1; i <= m; ++i) res += ret.g[i][i];
  return res % mod;
}
void solve() {
  int ans = 0;
  ...
  if (i * i == n) ans = (ans + phi(i) * gao(i)) % mod;
