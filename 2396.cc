  ...
  for (j = head[u]; j != -1; j = e[j].next) {
    ...
    if (val > 0) {
      if (dis[v] + 1 == dis[u]) {
        ...
        e[j].c -= d, e[j ^ 1].c += d;
        t -= d;
        if (dis[st] >= vn) return flow - t;
        if (t == 0) break;
      }
      if (dis[v] < mindis) mindis = dis[v];
    }
  }
  if (t == flow) {
    --gap[dis[u]];
    if (gap[dis[u]] == 0) dis[st] = vn;
    dis[u] = mindis + 1;
    ++gap[dis[u]];
  }
  return flow - t;
...
void solve() {
  ...
  for (int i = 1; i <= m; i++)
    for (int j = 1; j <= n; j++) matrix[i][j].c = INF;
  for (int i = 1; i <= m; i++) {
  ...
  while (q--) {
    ...
    if (str[0] == '=') {
      if (a && b) {
        if (c <= matrix[a][b].c && c >= matrix[a][b].l)
          matrix[a][b].c = matrix[a][b].l = c;
        else
          flag = true;
      }
      ...
      if (!a && !b) {
        for (int i = 1; i <= m; i++)
          for (int j = 1; j <= n; j++) {
            if (c <= matrix[i][j].c && c >= matrix[i][j].l)
              matrix[i][j].c = matrix[i][j].l = c;
            else
              flag = true;
          }
      }
      if (!a && b) {
        for (int i = 1; i <= m; i++) {
          if (c <= matrix[i][b].c && c >= matrix[i][b].l)
            matrix[i][b].c = matrix[i][b].l = c;
          else
            flag = true;
        }
      }
      if (a && !b) {
        for (int i = 1; i <= n; i++) {
          if (c <= matrix[a][i].c && c >= matrix[a][i].l)
            matrix[a][i].c = matrix[a][i].l = c;
          else
            flag = true;
        }
      }
  ...
  gap[0] = vn;
  while (dis[st] < vn) maxflow += isap(st, INF);
  if (maxflow != full) {
    printf("IMPOSSIBLE\n");
    return;
  }
