...
char grids[500][LL][LL];  // save the possible complete grid (no '.' only 'o' or
                          // 'x')
...
char patterns[19][10][10] = {{"xx",  // 1 kind -> 1 pattern
                              "xx"},
...
int kind_cnt[7] = {
    1, 2, 2, 4,
    4, 4, 2};  // kind count of the 7 kind of boats, i(begin with 0) kind jth
               // pattern is
               // patterns[kind_cnt[0]+kind_cnt[1]+......+kind_cnt[i-1]+j]
...
int insert_grid() {
  ...
  for (int i = 0; i <= R - 1; i++) {
    for (int j = 0; j <= C - 1; j++) {
      if (in_grid[i][j] == '.') {
        grids[cnt][i][j] = 'o';
      } else if (in_grid[i][j] >= 0 && in_grid[i][j] <= 6) {
        grids[cnt][i][j] = 'x';
      } else {
...
int DFS_kind(int cur) {
  ...
  if (cur >= 7) {
    ...
    {
      insert_grid();  // in_grid -> ('z','x'->'x'  'o','.'->'o') and not insert
                      // the repeat gird
    }
  } else {
      ...
      for (int nexti = 0; nexti <= R - 1; nexti++) {
        for (int nextj = 0; nextj <= C - 1; nextj++) {
          ...
          while (i <= 4 - 1 && j <= 4 - 1) {
            ...
            if (ch != '\0') {
              if (ch == 'x') {
                if (nexti + i > R - 1 || nextj + j > C - 1) {
                  break;
                }  // exceed boundary,it must no be matched
                if (in_grid[nexti + i][nextj + j] == 'x' ||
                    in_grid[nexti + i][nextj + j] == '.')  // matched
                {
                  if (in_grid[nexti + i][nextj + j] == '.') {
                    x_cnt++;
                  }
                  ...
                  match_cnt++;
                  in_grid[nexti + i][nextj + j] =
                      cur;  // the current cur turn we mark it to indicate that
                            // this point have been visited at curth turn
                } else {
                  break;
                }  // the in_grid[nexti+i][nextj+j]=='o' or 'z',not matched
              }
            }
            j++;
            if (j > 4 - 1) {
          ...
          // reverse the z -> x or .
          for (int ci = 0; ci <= match_cnt - 1; ci++) {
...
// dfs  the pot_grid and eliminate the various pot,the use_o must <= 1 , cur is
// the count of turn it must > 0
int DFS_pot(int cur, int use_o) {
  if (use_o >= 2) {
    return (0);
  }  // have used 'o' twice or more times then it is false
...
int main() {
  // note!! this problem in uva(322) is input is W H  but in poj(1138) is H W,
  // current version is for uva
  while (scanf("%d%d", &H, &W) != EOF && (W + H) > 0) {
    R = H;
    C = W;
    cnt = 0;
    x_cnt = o_cnt = i_cnt = 0;
    ...
    for (int i = 0; i <= R - 1; i++)  // initialize the pot_grid
    {
      for (int j = 0; j <= C - 1; j++) {
        pot_grid.val[i][j].is_various = false;
        pot_grid.val[i][j].m = pot_grid.val[i][j].n = pot_grid.val[i][j].x =
            pot_grid.val[i][j].y = 0;
      }
    }
    printf("Game #%d\n", ++cases);
    for (int i = 0; i <= R - 1; i++) {
    ...
    // dfs the input grid
    DFS_kind(
        0);  // place next patterns and insert the complete grid into the grids
