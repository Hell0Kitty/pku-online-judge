...
void heapify(int id) {
  ...
  if (l < size && heap[ii].value > heap[l].value) ii = l;
  if (r < size && heap[ii].value > heap[r].value) ii = r;
  if (ii != id) {
...
struct pp {
  double value;
  int id;
  bool operator<(const pp &temp) const { return value < temp.value; }
};
pp y[100];
struct point {
  double x, y;
...
struct train {
  ...
  bool operator<(const train &temp) const {
    if (p1 == temp.p1) return ru < temp.ru;
    return p1 < temp.p1;
  }
};
train trains[100];
bool inter(double x1, double y1, double x2, double y2, double x3, double y3,
           double x4, double y4) {
  double ru1, ru2;
  if ((x2 - x1) * (y4 - y3) - (y2 - y1) * (x4 - x3) == 0) return false;
  ru1 = ((x3 - x1) * (y4 - y3) - (y3 - y1) * (x4 - x3)) /
        ((x2 - x1) * (y4 - y3) - (y2 - y1) * (x4 - x3));
  ru2 = ((x3 - x1) * (y2 - y1) - (y3 - y1) * (x2 - x1)) /
        ((x2 - x1) * (y4 - y3) - (y2 - y1) * (x4 - x3));
  if (ru2 > -eps && ru2 < 1 + eps) {
    while (ru1 > 1 - eps || ru1 < eps) puts("orz");
...
double JiaJunpeng() {
  ...
  while (size) {
    ...
    u2 = heap[0].id2;
    u3 = heap[0].id3;
    if (u1 == cnt - 1) return dp[u1][u2][u3];
    heap[0] = heap[size - 1];
    size--;
    heapify(0);
    if (u1 != 0) {
          ...
          if (th1 - eps < dp[u1][u2][u3]) {
            ...
            for (s = 0; s < nu[id1][i]; s++) {
              if (dp[id1][i][s] <= value) {
                ...
                if (v <= times[id1][i].t2 + n2 * T - dp[id1][i][s]) break;
              }
            }
            if (s >= nu[id1][i]) {
...
int main() {
  ...
  double l;
  while (scanf("%d%d%lf%lf%lf%lf", &n, &m, &endx, &endy, &vt, &ve) &&
         (n != 0 || m != 0 && endx != 0 && endy != 0 && vt != 0 && ve != 0)) {
    for (i = 0; i < n; i++) scanf("%lf%lf", &poly[i].x, &poly[i].y);
    for (i = 0; i < m; i++) {
      scanf("%lf%lf%lf", &trains[i].hx, &trains[i].hy, &trains[i].len);
      while (trains[i].len == 0.00) puts("Orz");
    }
    len = 0.00;
    for (i = 0; i < n; i++) {
      id1 = i;
      ...
      for (j = 0; j < m; j++) {
        if ((trains[j].hy - poly[id1].y) * (poly[id2].x - poly[id1].x) ==
            (trains[j].hx - poly[id1].x) * (poly[id2].y - poly[id1].y)) {
          trains[j].p1 = id1;
          if (poly[id1].x != poly[id2].x)
            trains[j].ru =
                (trains[j].hx - poly[id1].x) / (poly[id2].x - poly[id1].x);
          else
            trains[j].ru =
                (trains[j].hy - poly[id1].y) / (poly[id2].y - poly[id1].y);
        }
      }
    }
    while (vt == 0.0 || ve == 0.0) puts("orz");
    T = len / vt;
    ...
    for (i = 0; i < m; i++) {
      ...
      ip = (id + 1) % n;
      trains[i].p2 = id;
      trains[i].tx = poly[id].x +
                     l * (poly[ip].x - poly[id].x) /
                         dist(poly[ip].x, poly[ip].y, poly[id].x, poly[id].y);
      trains[i].ty = poly[id].y +
                     l * (poly[ip].y - poly[id].y) /
                         dist(poly[ip].x, poly[ip].y, poly[id].x, poly[id].y);
    }
    cnt = 1;
    y[0].value = 0.00;
    ...
    for (i = 0; i < n; i++) {
      id1 = i;
      id2 = (i + 1) % n;
      if (inter(endx, 0, endx, endy, poly[id1].x, poly[id1].y, poly[id2].x,
                poly[id2].y))
        y[cnt++].id = (i + 1) % n;
    }
    sort(y + 1, y + cnt);
    ...
    for (i = 1; i < cnt - 1; i++) {
      ...
      for (j = 0; j < m; j++) {
        ...
        while (id != ip) {
          l += dist(poly[id].x, poly[id].y, poly[(id + 1) % n].x,
                    poly[(id + 1) % n].y);
          id = (id + 1) % n;
        }
        ...
        l += dist(poly[id].x, poly[id].y, endx, y[i].value);
        times[i][j].t2 = l / vt;
        while (times[i][j].t2 < times[i][j].t1) times[i][j].t1 -= T;
