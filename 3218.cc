...
void AlignCenter(TEXT paragraph) {
  while (!paragraph.empty()) {
    TEXT line = GetLine(paragraph);
    for (int i = line.size() + LineWordLength(line) - 1; i < LINEWIDTH - 1;
         i += 2)
      cout << ' ';
    PrintLine(line);
  }
  ...
  while (!paragraph.empty()) {
    ...
    if (paragraph.empty())
      ...
      while (!line.empty()) {
        ...
        line.pop();
        if (line.empty()) break;
        for (int i = 0; i < averageSpaceNum; i++) cout << ' ';
        if (outputCount < anotherSpaceNum) cout << ' ';
        outputCount++;
      }
      cout << endl;
    }
...
int main() {
  ...
  string word;
  TEXT paragraph;
  while (cin >> word) paragraph.push(word);
  switch (alignMode) {
    case 'L':
      AlignLeft(paragraph);
      break;
    case 'R':
      ...
      break;
    case 'C':
      AlignCenter(paragraph);
      break;
    case 'J':
      AlignJustify(paragraph);
      break;
    default:
      break;
  }
