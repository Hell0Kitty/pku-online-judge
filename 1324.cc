...
void input() {
  for (int i = 0; i < l; i++) scanf("%d%d", &point[i].x, &point[i].y);
  int t;
  scanf("%d", &t);
  ...
  while (!q.empty()) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (ok1(temp2)) {
        ...
        if (temp2.x == 1 && temp2.y == 1) return temp2.step;
        q.push(temp2);
      }
    }
  }
...
void setmap() {
  for (int i = 1; i < l; i++) map[point[i].x][point[i].y] = true;
}

void clearmap() {
  for (int i = 1; i < l; i++) map[point[i].x][point[i].y] = false;
}

int getmin() { return bfs1(); }
...
int getstate() {
  ...
  for (int i = 1; i < l; i++) {
    for (int j = 0; j < 4; j++)
      if (point[i].x == point[i - 1].x - dir[j][0] &&
          point[i].y == point[i - 1].y - dir[j][1]) {
        ret <<= 2;
        ret |= j;
      }
  }
