...
int main() {
  ...
  double p[4][4] = {{0, 0, 0, 1.0 / 16},
                    {1.0 / 16, 3.0 / 8, 1.0 / 8, 0},
                    {1.0 / 8, 3.0 / 16, 0, 0},
                    {1.0 / 16, 0, 0, 0}};
  ...
  for (int step = 1; step <= 21; step++) {
    for (map<int, double>::iterator ita = table[step - 1].begin();
         ita != table[step - 1].end(); ita++) {
      int p1 = getp1(ita->first), p2 = getp2(ita->first);
      if (step != 21) {
        for (int i = 0; i < 4; i++)
          for (int j = 0; j < 4; j++)
            table[step][encode(p1 + i, p2 + j)] +=
                (ita->second) * p[i][j];  //状态转移
      }
      if (p1 < p2)  //累加A输给B的概率
  ...
  for (int i = 1; i <= 20;
       i++)  //输出格式注意,如果需要在printf内输出%的话要用%%
    printf("%5d%10.4f%%%9.4f%%%9.4f%%\n", i, (1 - res[i][0] - res[i][1]) * 100,
           res[i][0] * 100, res[i][1] * 100);
  return 0;
}
