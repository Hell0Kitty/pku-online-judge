...
int cmp(Line l1, Line l2) {
  ...
  if (!Sig(l1.k - l2.k) && !Sig(l1.inter - l2.inter)) return l1.a.x < l2.a.x;
  if (!Sig(l1.k - l2.k)) return l1.inter < l2.inter;
  return l1.k < l2.k;
}
int Count(vector<Line> v) {
  ...
  for (int i = 0; i < n; i++) {
    v[i].inter =
        -(v[i].a.x) * (v[i].b.y - v[i].a.y) / (v[i].b.x - v[i].a.x) + v[i].a.y;
    v[i].k = (v[i].b.y - v[i].a.y) / (v[i].b.x - v[i].a.x);
  }
  sort(v.begin(), v.end(), cmp);
  int ans = 0;
  for (int i = 1; i < n; i++) {
    if (!Sig(v[i].inter - v[i - 1].inter) && !Sig(v[i].k - v[i - 1].k) &&
        Sig(v[i].a.x - v[i - 1].b.x) <= 0) {
      ans++;
...
int cmp1(Line l1, Line l2) {
  ...
  if (!Sig(l1.inter - l2.inter)) return l1.a.y < l2.a.y;
...
int main() {
  ...
  while (scanf("%d", &n) && n) {
    ...
    for (int i = 0; i < v1.size(); i++) {
      v1[i].inter = v1[i].a.x;
      if (v1[i].a.y > v1[i].b.y) swap(v1[i].a, v1[i].b);
    }
    sort(v1.begin(), v1.end(), cmp1);
    for (int i = 1; i < v1.size(); i++)
      if (!Sig(v1[i].inter - v1[i - 1].inter) &&
          Sig(v1[i].a.y - v1[i - 1].b.y) <= 0) {
        ans++;
        v1[i].b.y =
            max(v1[i].b.y, v1[i - 1].b.y);  //这一步处理同上一步的注释一个道理
      }
    printf("%d\n", n - ans);
  }
  return 0;
