...
int GetHash(int index) {
  int ans = 0;
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) ans = (ans * 131 + q[index].smap[i][j]) % mod;
...
int main() {
  while (scanf("%d%d%d", &n, &q[0].x, &q[0].y) && n) {
    ...
    for (int i = 0; i < n; i++) {
      getchar();
      for (int j = 0; j < n; j++) scanf("%c", &q[0].smap[i][j]);
    }
    ipos[GetHash(0)] = 1;
    int flag = 0, ans = 0;
    while (front != last) {
      char smap[8][8];
      for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) smap[i][j] = q[front].smap[i][j];
      int itx = q[front].x;
      int ity = q[front].y;
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (!(isborder(xx, yy) || smap[xx][yy] == '.')) {
          ...
          int hash = 0;
          for (int j = 0; j < n; j++)
            for (int k = 0; k < n; k++) hash = (hash * 131 + smap[j][k]) % mod;
          if (!ipos[(hash + xx * 33 + yy * 131) % mod]) {
            for (int j = 0; j < n; j++)
              for (int k = 0; k < n; k++) q[last].smap[j][k] = smap[j][k];
            q[last].x = xx;
            q[last].y = yy;
