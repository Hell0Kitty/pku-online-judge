...
int srcSubstrAns() {
    ...
    for(int i = 2; i <= len[src]; ++i) {
        ...
        for(int j = 0; j < n && flag; ++j) {
            if(findPos(buff, buff_len, text[j][0], len[j], pre[j][0]) == len[j] - 1) {
                continue;
            }
            if(findPos(buff, buff_len, text[j][1], len[j], pre[j][1]) == len[j] - 1) {
                continue;
            }
            flag = false;
        }
...
int dfs(int stat, int last, int inv) {
    ...
    for(int i = 0; i < n; ++i) {
        if(stat & (1 << i)) {
            if(pos[last][inv][i][0] == len[i] - 1 || pos[last][inv][i][1] == len[i] - 1) {
                stat ^= (1 << i);
            }
        }
    }
    ...
    for(int i = 0; i < n; ++i) {
        if(stat & (1 << i)) {
            val = std::min(val, dfs(stat ^ (1 << i), i, 0) + len[i] - (pos[i][0][last][inv] + 1));
            val = std::min(val, dfs(stat ^ (1 << i), i, 1) + len[i] - (pos[i][1][last][inv] + 1));
        }
    }
    return val;
}
    ...
    while(scanf("%d", &n) == 1 && n) {
        ...
        for(int i = 0; i < n; ++i) {
            for(int p = 0; p < 2; ++p) {
                for(int j = 0; j < n; ++j) {
                    for(int q = 0; q < 2; ++q) {
                        pos[i][p][j][q] = findPos(text[i][p], len[i], text[j][q], len[j], pre[j][q]);
                    }
                }
            }
        }
