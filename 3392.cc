...
void dfs(int pos) {
  if (pos == -1) {
    dp[to][num[1]][num[2]][num[3]][num[4]] = max(
        dp[to][num[1]][num[2]][num[3]][num[4]],
        dp[from][num[0]][num[1]][num[2]][num[3]] + em[fight[day]][num[4]].val);
    return;
  }
  for (num[4 - pos] = 1; num[4 - pos] <= 5; num[4 - pos]++) {
    if (vis[em[fight[day - pos]][num[4 - pos]].pos] > 0) continue;
...
int main() {
  ...
  for (t = 1; t <= T; t++) {
    ...
    for (i = 1; i <= m; i++) sort(em[i] + 1, em[i] + 1 + n, cmp);
    n = min(n, 5);
    for (i = 11; i <= g + 20; i++) scanf("%d", &fight[i]);
    ...
    for (a = 0; a <= 5; a++)
      for (b = 0; b <= 5; b++)
        for (c = 0; c <= 5; c++)
          for (d = 0; d <= 5; d++) ans = max(ans, dp[to][a][b][c][d]);
    printf("%.2f\n", 0.01 * ans);
  }
}
