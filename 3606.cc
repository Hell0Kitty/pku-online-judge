...
const int mod = 9973;
int n, C, op[10005], f00[20005], f10[20005], f01[20005], f11[20005], f0[20005],
    f1[20005];
  ...
  for (scanf("%d", &C); C--;) {
    ...
    for (i = n - 1; i >= 1; i--) {
      ...
      if (op[i]) {
        ...
        f00[i] = (f1[p1] * f00[p2] + f0[p1] * (f0[p2] + f1[p2] - 1) + f00[p1] +
                  f01[p1]) %
                 mod;
        ...
        f11[i] = (f1[p1] * (f0[p2] + f1[p2] - 1) + f0[p1] * f11[p2] + f01[p1] +
                  f11[p1]) %
                 mod;
      }
    }
