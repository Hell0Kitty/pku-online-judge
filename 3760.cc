...
void worriors::init(int input) { total[0] = total[1] = input; }
...
void worriors::get_hp(int a) { hp += a; }
dragon::dragon(int newcolor, int newid, int newtype)
    : worriors(newcolor, newid, newtype) {}
void dragon::happy(int _id) {
  printf("%.3d:40 %s dragon %d yelled in city %d\n", now_time, s[color], id,
         _id);
}
ninjia::ninjia(int newcolor, int newid, int newtype)
    : worriors(newcolor, newid, newtype) {}
iceman::iceman(int newcolor, int newid, int newtype)
    : worriors(newcolor, newid, newtype) {}
lion::lion(int newcolor, int newid, int newtype)
    : worriors(newcolor, newid, newtype) {}
wolf::wolf(int newcolor, int newid, int newtype)
    : worriors(newcolor, newid, newtype) {}
void wolf::twice() {
  if (kill > 0 && kill % 2 == 0) {
    hp *= 2;
...
void city::beat() {
  ...
  printf("%.3d:40 %s %s %d ", now_time, s[turn], ss[data[turn]->type],
         data[turn]->id);
  printf("attacked %s %s %d in city %d with %d elements and force %d\n",
         s[turn2], ss[data[turn2]->type], data[turn2]->id, id, data[turn]->hp,
         data[turn]->attack);
  if (data[turn2]->type == LION) left_hp = data[turn2]->hp;
  data[turn2]->hp -= data[turn]->attack;
  if (data[turn2]->hp <= 0) {
    printf("%.3d:40 %s %s %d was killed in city %d\n", now_time,
           s[data[turn2]->color], ss[data[turn2]->type], data[turn2]->id, id);
    delete data[turn2];
    data[turn2] = NULL;
    data[turn]->kill++;
    data[turn]->twice();
    if (left_hp != 0) data[turn]->hp += left_hp;
    data[turn]->happy(id);
    ...
    if (data[turn2]->type != NINJIA) {
      printf("%.3d:40 %s %s %d fought back against %s %s %d in city %d\n",
             now_time, s[turn2], ss[data[turn2]->type], data[turn2]->id,
             s[turn], ss[data[turn]->type], data[turn]->id, id);
      ...
      if (data[turn]->hp <= 0) {
        printf("%.3d:40 %s %s %d was killed in city %d\n", now_time,
               s[data[turn]->color], ss[data[turn]->type], data[turn]->id, id);
        delete data[turn];
        data[turn] = NULL;
        if (left_hp != 0) data[turn2]->hp += left_hp;
        printf("%.3d:40 %s %s %d earned %d elements for his headquarter\n",
               now_time, s[turn2], ss[data[turn2]->type], data[turn2]->id, hp);
...
bool city::reach_headquater(int _color) {
  if (data[_color] == NULL) return false;
  printf(
      "%.3d:10 %s %s %d reached %s headquarter with %d elements and force %d\n",
      now_time, s[_color], ss[data[_color]->type], data[_color]->id,
      s[1 - _color], data[_color]->hp, data[_color]->attack);
...
void city::print_last_go() {
  for (int i = 0; i <= 1; i++)
    if (data[i] != NULL)
      printf(
          "%.3d:10 %s %s %d marched to city %d with %d elements and force %d\n",
          now_time, s[i], ss[data[i]->type], data[i]->id, id, data[i]->hp,
          data[i]->attack);
}
void creat() {
  for (int i = 0; i <= 1; i++) {
...
void go_ahead() {
  for (int i = city_sum + 1; i >= 1; i--) citys[i].go(&citys[i - 1], 0);
  for (int i = 0; i <= city_sum; i++) citys[i].go(&citys[i + 1], 1);
  citys[0].data[0] = NULL;
  ...
  for (int i = 1; i <= city_sum; i++) citys[i].print_last_go();
...
int main() {
  ...
  for (int t = 1; t <= times; t++) {
    ...
    for (int i = 0; i <= 4; i++) scanf("%d", &_attack[i]);
    printf("Case:%d\n", t);
    for (int i = 0; i <= city_sum + 1; i++) citys[i].creat(i);
    while (1) {
      now_time++;
      creat();
      already_time += 10;
      ...
      go_ahead();
      if (game_over) break;
      for (int i = 1; i <= city_sum; i++) citys[i].input_hp();
      already_time += 20;
      if (already_time > total_time) break;
      for (int i = 1; i <= city_sum; i++) citys[i].output_hp();
      already_time += 10;
      if (already_time > total_time) break;
      for (int i = 1; i <= city_sum; i++) citys[i].beat();
      pride();
      already_time += 10;
      if (already_time > total_time) break;
      for (int i = 0; i <= 1; i++)
        printf("%.3d:50 %d elements in %s headquarter\n", now_time,
               worriors::total[i], s[i]);
      already_time += 10;
      if (already_time > total_time) break;
    }
  }
