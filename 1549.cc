...
int ans[15];
int mark[15];
int state[4500][8][8];    //在第一维压缩记录状态，用到哪个镯子就把相应位置1，得到的整数即是第一维的索引
...
int main()
{
 ...
 for (int i=1;i<=11;++i)
  mi[i]=mi[i-1]<<1;
         ...
         if (state[x][q][p]!=-1)  //前状态存在
         {
                   ...
                   for (int k=0;k<8;k++)
          {
           ...
           if (p==l)  //插在右端
           {
            ...
            if (state[mi[s]+x][q][r]==-1 || state[x][q][p]+b[p]<state[mi[s]+x][q][r])
             state[mi[s]+x][q][r]=state[x][q][p]+b[p];
           }
           if (q==r)  //插在左端
           {
            if (i==n-1 && l!=p) continue;
            if (state[mi[s]+x][l][p]==-1 || state[x][q][p]+b[q]<state[mi[s]+x][l][p])
    ...
    int Min=inf;
    for (int i=0;i<8;i++)
      if (state[tot][i][i]!=-1 )  //因为镯子要成环形，最后的状态要满足头尾颜色也相等
