...
void read() {
  ...
  int i, j;
  for (i = 0; i < N; i++) {
    for (j = 0; j < M; j++) scanf("%d", &tran[i][j]);
    queue[0][i] = i;
  }
}
...
bool checkCurState(int front, int size) {
  for (int i = 0; i < size; i++)
    if (queue[front][i] != 0) return false;
  return true;
}
...
bool checkHaveUsed(int cur, int curState[], int size) {
  for (int x = cur; x != 0; x = cmdList[x].pre) {
    bool flag = false;
    for (int i = 0; i < size; i++)
      if (curState[i] != queue[x][i]) flag = true;
    if (!flag) return true;
  }
  return false;
}
...
bool bfs() {
  ...
  while (front < rear) {
    ...
    for (int i = 0; i < M; i++) {
      for (int j = 0; j < N; j++) queue[rear][j] = tran[queue[front][j]][i];
...
void print(int size) {
  for (int i = size - 1; i >= 0; i--) printf("%c", xx[cmds[i]]);
  putchar('\n');
}
