...
int between1(point p0, point p1, point p2) { return sig(dot1(p0, p1, p2)); }
...
double getdis(int a, int b) {
  ...
  for (int i = 0; i < cnts[a]; i++)
    for (int j = 0; j < cnts[b]; j++)
      if (intersect1(p[a][i], p[a][(i + 1) % cnts[a]], p[b][j],
                     p[b][(j + 1) % cnts[b]]))
        return 0;
  for (int i = 0; i < cnts[a]; i++)
    for (int j = 0; j < cnts[b]; j++) dis = min(dis, dist(p[a][i], p[b][j]));
  for (int i = 0; i < cnts[a]; i++)
    for (int j = 0; j < cnts[b]; j++)
      dis = min(dis,
                dis_point_segment(p[a][i], p[b][j], p[b][(j + 1) % cnts[b]]));
  for (int i = 0; i < cnts[b]; i++)
    for (int j = 0; j < cnts[a]; j++)
      dis = min(dis,
                dis_point_segment(p[b][i], p[a][j], p[a][(j + 1) % cnts[a]]));
  return dis;
}
...
void initDis() {
  ...
  for (int i = 1; i <= n; i++) {
...

    {
      ...
      diss1 =
          min(diss1, dis_point_segment(s, p[i][j], p[i][(j + 1) % cnts[i]]));
    }
    sd[i] = diss1;
  }
  for (int i = 1; i <= n; i++) {
