...
sp operator*(double d, const sp &v) { return sp(d * v.x, d * v.y, d * v.z); }
...
double norm(const sp &v) { return sqrt(v.x * v.x + v.y * v.y + v.z * v.z); }

double normsqr(const sp &v) { return v.x * v.x + v.y * v.y + v.z * v.z; }

double dis(const sp &u, const sp &v) { return norm(u - v); }
...
double dis(const sp &p, const ss &s) {
  ...
  if (online(q, s))
    ...
    return min(dis(s.a, p), dis(s.b, p));
}
...
int main() {
  ...
  while (~scanf("%lf%lf%lf", &p[0].x, &p[0].y, &p[0].z)) {
    for (i = 1; i < 4; ++i) p[i].read();
    org.read();
    scanf("%lf", &d);
    seg[0] = ss(p[0], p[1]);
    seg[1] = ss(p[2], p[3]);
    sp hmr, mdk;
    if (dis(seg[0].a, org) < d + EPS)
      s = seg[0].a;
    else {
      hmr = (d / norm(seg[0].a - org)) * (seg[0].a - org);
      s = org + hmr;
    }
    if (dis(seg[0].b, org) < d + EPS)
      t = seg[0].b;
    else {
