...
void walk(int i, int j, int x0, int y0, int x, int y, int z, int l, int w,
  ...
  if (z == 0)
    ...
    if (i >= 0 && i < 2) walk(i + 1, j, x0, y0 - w, x, z, w - y, l, h, w);
    if (i <= 0 && i > -2) walk(i - 1, j, x0, y0 + h, x, h - z, y, l, h, w);
    if (j >= 0 && j < 2) walk(i, j + 1, x0 - l, y0, z, y, l - x, h, w, l);
    if (j <= 0 && j > -2) walk(i, j - 1, x0 + h, y0, h - z, y, x, h, w, l);
  }
}
