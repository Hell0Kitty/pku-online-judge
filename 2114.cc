...
inline int getcend(int k, int yff) {
  ...
  for (iter i = P[k].begin(); i != P[k].end(); ++i) {
    Max = yff - size[*i];
    for (int j = a[*i]; j; j = E[j].next)
      if (p[*i] != E[j].v) innew(Max, size[E[j].v]);
  ...
  for (iter i = P[k].begin(); i != P[k].end(); ++i) {
    ...
    while (st < ed) {
      ...
      for (int i = a[x]; i; i = E[i].next)
        if (!dis[E[i].v]) dis[q[++ed] = E[i].v] = dis[x] + E[i].w;
    }
  ...
  for (int need = 1; need <= nsk; ++need) {
    ...
    for (int i = 1; i <= ttot; ++i) {
      ...
      if (tlist[j] + tlist[i] == ask[need] + 2) {
        if (j != i)
          ans[need] += plist[i] * plist[j];
        else
          ans[need] += plist[i] * (plist[i] - 1);
      }
    }
  }
    ...
    for (int need = 1; need <= nsk; ++need) {
      ...
      for (int w = 1; w <= ttot; ++w) {
        ...
        if (tlist[w] + tlist[j] == ask[need] + 2) {
          if (j != w)
            ans[need] -= plist[w] * plist[j];
          else
            ans[need] -= plist[w] * (plist[w] - 1);
        }
      }
    }
