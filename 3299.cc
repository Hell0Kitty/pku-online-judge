...
int getid(char *st) {
  if (strcmp(st, "T") == 0) return 0;
  if (strcmp(st, "D") == 0) return 1;
  return 2;
}
...
int main() {
  ...
  while (scanf("%s", st) != EOF && !(strcmp(st, "E") == 0)) {
    ...
    data[getid(st)] = a;
    getchar();
    if (data[0] == 0) calt(data[0], data[1], data[2]);
    if (data[1] == 0) cald(data[0], data[1], data[2]);
    if (data[2] == 0) calh(data[0], data[1], data[2]);
