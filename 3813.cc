...

{
  ...
  while (scanf("%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf", &a.x, &a.y, &b.x, &b.y,
               &c.x, &c.y, &d.x, &d.y, &e.x, &e.y, &f.x, &f.y) != EOF)

  {
    if (!a.x && !a.y && !b.x && !b.y && !c.x && !c.y && !d.x && !d.y && !e.x &&
        !e.y && !f.x && !f.y)
...

    {
      ...
      {
        H.x = (h * sqrt(lA * lA + lB * lB) - lC + (lB * lC1 / lB1)) /
              ((lA * lB1 - lB * lA1) / lB1);

        if (H.x < a.x)

          H.x = ((-1) * h * sqrt(lA * lA + lB * lB) - lC + (lB * lC1 / lB1)) /
                ((lA * lB1 - lB * lA1) / lB1);
