...
Vector a[5];
int cmp(Vector a, Vector b) { return a.ang < b.ang; }
int main() {
  int n;
  ...
  while (n--) {
    ...
    double cx = ((y2 - y1) * (y3 * y3 - y1 * y1 + x3 * x3 - x1 * x1) -
                 (y3 - y1) * (y2 * y2 - y1 * y1 + x2 * x2 - x1 * x1)) /
                (2 * (x3 - x1) * (y2 - y1) - 2 * ((x2 - x1) * (y3 - y1)));
    double cy = ((x2 - x1) * (x3 * x3 - x1 * x1 + y3 * y3 - y1 * y1) -
    ...
    for (int i = 3; i <= 200; i++) {
      ...
      if (fabs(s1 - b1) < 1e-5 && fabs(s2 - b2) < 1e-5 &&
          fabs(s3 - b3) < 1e-5) {
        ans = i;
        break;
      }
