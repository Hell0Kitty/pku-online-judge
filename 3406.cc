...
int g(int n,
      int x)  //小于等于n的奇数序列中x的幂（或x出现的次数）,递归实现（美妙）
{
  if (n == 0) return 0;
  return n / 10 + (n % 10 >= x ? 1 : 0) +
         g(n / 5,
           x);  //例如g(27,3),取十位数表明之前x循环出现2次，个位7>=3表示x又出现一次
}
...
int solve() {
  ...
  if (pow_2 < pow_5)
    return 5;  //如果是n!,i>k，这里是组合数不一定，要判断
  else if (pow_2 == pow_5)
    ans = 1;
  else
    ans *= table[0][(pow_2 - pow_5) % 4];
  // cout<<getpow(n,5)<<endl;
  // cout<<getpow(n-m,5)<<endl;
  //   cout<<ans<<endl;
  /*****再处理末尾3,5,9************/  // n!个位数除完所有2、5后末尾只可能是1,3,7,9
  int pow_3 = f(n, 3) - f(n - m, 3) - f(m, 3);
  pow_3 = (pow_3 % 4 + 4) % 4;
