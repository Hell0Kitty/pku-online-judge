  ...
  for (k = 0; k < hm[cur].size; k++) {
    ...
    if (left && up) {
      ...
      if (maze[i][j + 1] && maze[i + 1][j]) {
        code[j - 1] = code[j] = 13;
        hm[cur ^ 1].push(encode(code, M),
                         hm[cur].dp[k] + node[i][j].down + node[i][j].right);
      }
    }
  }
}
...
void init() {
  ...
  for (int i = 1; i <= N; i++)
    for (int j = 1; j <= M; j++) maze[i][j] = 1;
  gets(str);
  for (int i = 1; i < N; i++) {
    gets(str);
    for (int j = 1; j < M; j++) node[i][j].right = str[2 * j] - '0';
    gets(str);
    for (int j = 1; j <= M; j++) node[i][j].down = str[2 * j - 1] - '0';
  }
  gets(str);
  for (int j = 1; j < M; j++) node[N][j].right = str[2 * j] - '0';
  gets(str);
  ex = N;
  ey = M;
}
void solve() {
  ...
  for (i = 0; i < hm[cur].size; i++) ans += hm[cur].dp[i];
  printf("%d\n", ans);
}
int main() {
  int T;
