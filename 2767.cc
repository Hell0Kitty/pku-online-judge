...
double help(int RR,int rr,int mm){
	return 1.0*(mm-rr)/(RR-rr);
}
void gao(){
	double hi[10];
	hi[0]=0;
	for(int i=1;i<n;i++){
		...
		for(int j=0;j<i;j++){
			...
			if(R[p[j]]>=r[p[i]]&&R[p[j]]<=R[p[i]]){
				th=max(th,(1-help(R[p[i]],r[p[i]],R[p[j]]))*h[p[i]]+h[p[j]]+hi[j]-h[p[i]]);
			}
			//kazhong
			if(R[p[j]]>=R[p[i]]&&r[p[j]]<=R[p[i]]){
				th=max(th,help(R[p[j]],r[p[j]],R[p[i]])*h[p[j]]+hi[j]-h[p[i]]);
			}
			//kaxia
			if(R[p[j]]>=r[p[i]]&&r[p[j]]<=r[p[i]]){
