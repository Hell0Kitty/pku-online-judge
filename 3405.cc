...
int graham(Point* p, int n, Point* ch) {
  ...
  for (int i = 2; i < n; ++i) {
    while (top && dcmp(Cross(ch[top] - ch[top - 1], p[i] - ch[top - 1])) <= 0)
      --top;
    ch[++top] = p[i];
  }
  int len = top;
  ...
  for (int i = n - 3; i >= 0; --i) {
    while (top > len &&
           dcmp(Cross(ch[top] - ch[top - 1], p[i] - ch[top - 1])) <= 0)
      --top;
    ch[++top] = p[i];
  }
  return top;
  ...
  while (~scanf("%d", &m)) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      if (r > eps)
        ...
        point[nn].x = x, point[nn].y = y, point[nn++].id = -1;
    }
    cc[n] = cc[0];
    if (nn > 0) {
      for (int i = 0; i < nn; i++) {
        pp[cnt++] = point[i];
        for (int j = 0; j < n; j++) getTangentPoints(point[i], cc[j], pp, j);
      }
    }
    double maxr = 0;
    for (int i = 0; i < n; i++) {
    ...
    for (int i = 0; i < cnt_cv; i++) {
      ...
      if (res[i].id == res[i + 1].id && res[i].id != -1) {
        ...
        area += 0.5 * tmp * cc[res[i].id].r;
                 ...
                 res[i].id != res[i + 1].id) {
        area +=
            getarea2(cc[res[i].id].c, res[i], res[i + 1], cc[res[i + 1].id].c);
      } else if (res[i].id != -1 && res[i + 1].id == -1) {
        double tmp = Cross(res[i], res[i + 1]) +
                     Cross(res[i + 1], cc[res[i].id].c) +
                     Cross(cc[res[i].id].c, res[i]);
        area += tmp * 0.5;
      } else if (res[i].id == -1 && res[i + 1].id != -1) {
        double tmp = Cross(res[i], res[i + 1]) +
                     Cross(res[i + 1], cc[res[i + 1].id].c) +
                     Cross(cc[res[i + 1].id].c, res[i]);
        area += tmp * 0.5;
      } else
        continue;
    }
    ...
    if (area - tmp_area < eps) area = tmp_area;
    printf("%.4lf\n", area);
    st.clear();
  }
  return 0;
