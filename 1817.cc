...
// 0表示下  1表示右
inline int GetDir(char a)
{
	return ( (a & (1 << 7)) ? 1 : 0 );  
}

inline int GetLen(char a)
{
	return ( (a & (1 << 6)) ? 3 : 2 );  
}

inline int GetRow(char a)
{
	return( (a >> 3) & 7);
}

inline int GetVec(char a)
{
	return(a & 7);
}
...
int main()
{
	...
	while(cin >> n)
	{
		...
		for(j = GetVec(nd.box[target_index]) + GetLen(nd.box[target_index]); j < 6; ++j)
		{
			if(vehicles[target_row][j] != '.' && GetDir(nd.box[vehicles[target_row][j] - 'a']) == 1)
			{
