...
void build(int k, say tem[]) {
  if (k < 6) {
    if (brith[k]) {
      for (people[k] = 1; people[k] < 4; people[k]++) build(k + 1, tem);
    } else
      build(k + 1, tem);
  } else if (judge(tem)) {
    //		cout << k << endl;
    if (ok >= 1)
      ...
      for (int i = 0; i < 6; i++) now[i] = people[i];
    ok++;
  }
}
...
int main() {
  ...
  while (cin >> n, n) {
    ...
    //		else
    //		{
    for (people[0] = 0; people[0] < 2; people[0]++) build(1, tem);
    ...
    if (ok == 0)
      ...
      if (oi == 0 && yes[0]) cout << "No facts are deducible." << endl;
      if (now[0] && !yes[0])
        cout << "It is day." << endl;
      else if (!yes[0])
