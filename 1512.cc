...
void solve(int top,int now,int l,int r,int bot) {
	...
	if (ki[now]) {
		...
		tim[top+1]++;solve(top+1,p[now][0],(l+2)>>1,(r+1)>>1,0);
		j=((r+1)>>1)-((l+2)>>1)+1;
		FOR(i,1,j) ans[top][bot+i+i-( (l&1))]=ans[top+1][i];
		tim[top+1]++;solve(top+1,p[now][1],(l+1)>>1,(r+0)>>1,0);
		j=((r+0)>>1)-((l+1)>>1)+1;
		FOR(i,1,j) ans[top][bot+i+i-(!(l&1))]=ans[top+1][i];
	} else {
		q[top][now]=tim[top];
		if (l>a[now][0]) {
			solve(top,nex[now],l-a[now][0],r-a[now][0],bot);return ;
		}
		if (r<=a[now][0]) {
			FOR(i,l,r) ans[top][bot+(i-l+1)]=a[now][i];return ;
		}
...
int main() {
	int i,j,k; char na,chi,chj;
	scanf("%d%d",&n,&m);
	for (;n;n--) {
		scanf("\n%c",&na);na-='A';
		if (jump(3),cin.peek()=='z') {
			scanf("zip %c %c",&chi,&chj);
			ki[na]=1;p[na][0]=int(chi-'A');p[na][1]=int(chj-'A');
		} else {
			for (;;) {
				if (isdigit(cin.peek())) {
					scanf("%d",&a[na][++a[na][0]]);
