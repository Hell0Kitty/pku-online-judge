...
int search(char *city) {
  for (int i = 0; i < C; i++) {
    if (!strcmp(name[i], city)) return i;
  }
  return -1;
}
void solve(int dep) {
  ...
  for (k = 0; k < C; k++) {
    for (i = 0; i < T; i++) {
      for (j = 0; j < train[i] - 1; j++) {
        if (map[i][j].time >= travel[map[i][j].city] &&
  ...
  for (i = 0; i < C; i++) {
    if (i != dist) travel[i] = -1;
  }
  for (k = 0; k < C; k++) {
    for (i = 0; i < T; i++) {
      for (j = 0; j < train[i] - 1; j++) {
        if (map[i][j + 1].time <= travel[map[i][j + 1].city] &&
