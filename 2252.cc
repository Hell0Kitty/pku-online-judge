...
int OK1(char x) { return x >= '0' && x <= '9'; }

int OK2(char x) { return x == 'x'; }

int OK3(char x) { return x == '+' || x == '-'; }
  ...
  for (i = s; i <= e; i++) {
    if (OK1(str[i]) || OK2(str[i]) || str[i] == '(') {
      ...
      if (OK2(str[i]))
        ...
        temp.a = str[i] - '0';
        for (i++; i <= e && OK1(str[i]); i++)
          temp.a = temp.a * 10 + str[i] - '0';
        i--;
      } else {
        pos = find(i);
        temp = DFS(i + 1, pos - 1);
      ...
      while (1) {
        ...
        if (str[i] == '(') {
          ...
          temp1 = DFS(i + 1, pos - 1);
          if (temp1.b == 0)
            temp.a *= temp1.a, temp.b *= temp1.a;
          else
            temp.b = temp.a * temp1.b, temp.a = temp.a * temp1.a;
          i = pos;
          continue;
        }
        if (str[i] == 'x')
