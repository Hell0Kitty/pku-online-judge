...
void intersection(location A, location B, location C, location D) {
  ...
  xx = (area2 * C.x - area1 * D.x) /
       (area2 -
        area1);  //本题所求的交点一定是规范相交所得，因此无需判断是否规范相交
...
double area(location A, location B, location C, location D) {
  double triangle1 =
      fabs(0.5 * cross(A, B, A, C));  //用计算几何的方法计算的面积是有向面积
  double triangle2 = fabs(
      0.5 * cross(A, B, A, D));  //即算出来的面积可能为负数，因此必须用绝对值
                                 // fabs() 为取double的绝对值函数
...
int main(int i, int j, int k) {
  ...
  while (cin >> n) {
    ...
    for (i = 1; i <= n + 1; i++)
      for (j = 1; j <= n + 1; j++) {
        double temp = area(node[i - 1][j - 1], node[i][j], node[i][j - 1],
                           node[i - 1][j]);
