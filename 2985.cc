...
struct Treap {
  ...
  void Update(int x) { sz[x] = cnt[x] + sz[son[x][0]] + sz[son[x][1]]; }
  ...
  void _Erase(int &x, int k) {
    if (key[x] == k) {
      ...
      _Erase(son[x][k > key[x]], k);
    Update(x);
  }
  int _Get_kth(int &x, int k) {
    if (k <= sz[son[x][0]]) return _Get_kth(son[x][0], k);
    k -= sz[son[x][0]] + cnt[x];
    if (k <= 0) return key[x];
  ...
  int Get_kth(int k) { return _Get_kth(root, k); }
} tp;
