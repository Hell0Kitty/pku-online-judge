...
{
    if (ope == 200) return a * b;
    else if (ope == 100) return a + b;
    else return b - a;
}
...
int slove(string &s)
{
    ...
    for (int i = 0; i < vec[t].size(); ++i)
    {
        ...
        if (l == 150 && vec[t][i+1] >= '0' && vec[t][i+1] <= '9')
            l = 10;
        if (l < 50)
        {
            if (l == 10)
            ...
            {
                string sl = "";
                while (i < vec[t].size() && vec[t][i] != ' ' && get_val(vec[t][i]) < 50) sl += vec[t][i++];
                --i;
                int ml = slove(sl);
        ...
        else
        {
            ...
            if (l == 50) ope.pop_back();
            else
            {
                if (ope.size() > 0 && ope.back() == 150)
                {
                    if (l == 100) ope.push_back(150);
                    else if (l == 150) ope.push_back(100);
                    else ope.push_back(l);
                }
                else ope.push_back(l);
            }
        }
