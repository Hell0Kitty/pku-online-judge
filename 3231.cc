...
void cal_time() {
  for (int i = 0; i < n; i++)
    if (!finished[i]) time_left[i] = elem[i].size / elem[i].speed;
}

void size_reduce(double min_time) {
  for (int i = 0; i < n; i++)
    if (!finished[i]) elem[i].size -= min_time * elem[i].speed;
}

int get_min() {
  int ret = 0;
  while (finished[ret]) ret++;
  for (int i = ret + 1; i < n; i++)
    if (!finished[i] && time_left[i] < time_left[ret]) ret = i;
  return ret;
}
...
void distribute(double bandwidth) {
  ...
  while (band_left > 0) {
    ...
    bool did = false;
    for (int i = 0; i < n; i++) {
      if (finished[i] || reached[i]) continue;
      did = true;
...
int main() {
  ...
  while (scanf("%d", &n), n) {
    ...
    input();
    work();
    for (int i = 0; i < n; i++) printf("NO%d:%.3fs\n", i + 1, ans[i]);
  }
  return 0;
}
