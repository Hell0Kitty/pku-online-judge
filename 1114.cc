...
void insert(char *s, int k, P *p) {
  int i;
  for (i = 0; i < num; i++) {
    if (!strcmp(s, p[i].s)) break;
  }
  if (i < num)
    p[i].n += k;
  else
    p[num].n += k, strcpy(p[num++].s, s);
}
  ...
  while (i < t) {
    if ('(' == str[i]) {
      ...
      if ('0' <= str[ti] && str[ti] <= '9') {
        ...
        element(str, i, ti, k, p);
      i = ti;
    } else if ('A' <= str[i] && str[i] <= 'Z') {
      int tk = i;
      i++;
      if ('a' <= str[i] && str[i] <= 'z') i++;
      if ('0' <= str[i] && str[i] <= '9') {
        int ttk;
  ...
  while (EOF != scanf("%s", str)) {
    ...
    while (n--) {
      ...
      if (m == num) {
        ...
        int i;
        for (i = 0;
             i <= num && node[i].n == get[i].n && !strcmp(node[i].s, get[i].s);
             i++)
          ;
        if (i > num) {
          printf("%s==%s\n", str, str1);
