...
void input()
{
	...
	sum=0;
	for(i=1;i<=50;i++) for(j=1;j<=n;j++) if(a[j][i]!=-1)
		all[++sum]=a[j][i];
}
...
Pos findPos(int bookID)
{
	int i,j;
	Pos ans;
	for(i=1;i<=m;i++) for(j=1;j<=a[i].num;j++) if(a[i].a[j].id==bookID)
	{
		ans.deskID=i;
		ans.pos=j;
		return ans;
...
{
	int i;
	for(i=2;i<=m;i++) if(a[i].num<c) return i;
	return m+1;
}
...
Pos findMin()
{
	...
	Pos ans;
	ans.deskID=1;
	for(i=1;i<=a[1].num;i++) if(a[1].a[i].time<min)
	{
		min=a[1].a[i].time;
		ans.pos=i;
	}
