...
inline double ptDis(Point a, Point b) {
...
inline double angle(Vec a, Vec b) {
  return acos(dotDet(a, b) / vecLen(a) / vecLen(b));
}
inline double triArea(Point a, Point b, Point c) {
  return fabs(crossDet(b - a, c - a));
}
inline Vec vecUnit(Vec x) { return x / vecLen(x); }
inline Vec rotate(Vec x, double rad) {
...
inline bool onSeg(Point x, Point a, Point b) {
  return sgn(crossDet(a - x, b - x)) == 0 && sgn(dotDet(a - x, b - x)) < 0;
}
...
inline int segIntersect(Seg a, Seg b) {
...
inline Point lineIntersect(Line a, Line b) {
  return lineIntersect(a.s, a.t - a.s, b.s, b.t - b.s);
}
...
int andrew(Point *pt, int n, Point *ch) {
  ...
  int m = 0;
  for (int i = 0; i < n; i++) {
    while (m > 1 && crossDet(ch[m - 1] - ch[m - 2], pt[i] - ch[m - 2]) <= 0)
      m--;
    ch[m++] = pt[i];
  }
  int k = m;
  for (int i = n - 2; i >= 0; i--) {
    while (m > k && crossDet(ch[m - 1] - ch[m - 2], pt[i] - ch[m - 2]) <= 0)
      m--;
    ch[m++] = pt[i];
  }
  if (n > 1) m--;
...
inline bool cmpDis(Point p1, Point p2) {
  return ptDis(p1, origin) > ptDis(p2, origin);
}

void removePt(Point *pt, int &n) {
  int idx = 1;
  for (int i = 2; i < n; i++) {
    if (sgn(crossDet(origin, pt[i], pt[idx])))
      pt[++idx] = pt[i];
    else if (cmpDis(pt[i], pt[idx]))
      pt[idx] = pt[i];
  }
  n = idx + 1;
}
...
inline Poly cutPoly(Poly &poly, DLine L) {
  return cutPoly(poly, L.p, L.p + L.v);
}
...
// 3D Geometry
void getCoor(double R, double lat, double lng, double &x, double &y,
             double &z) {
  lat = toRad(lat);
  lng = toRad(lng);
  x = R * cos(lat) * cos(lng);
...
int main() {
  ...
  while (cin >> n) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (segIntersect(ori[i], ori[i + 1], ori[j], ori[j + 1]) == 1) {
          ex.push_back(lineIntersect(Line(ori[i], ori[i + 1]),
                                     Line(ori[j], ori[j + 1])));
        }
      }
    }
    ...
    for (int i = 0; i < sz; i++) {
      for (int j = 0; j < n; j++) {
        if (ex[i] == ori[j] || ex[i] == ori[j + 1] ||
            onSeg(ex[i], Seg(ori[j], ori[j + 1])))
          on[j][++on[j][0]] = i;
      }
    }
    ...
    for (int i = 0; i < psz; i++) {
      if (i > 0 &&
          sgn(crossDet(ex[path[i]] - ex[path[i - 1]],
                       ex[path[i + 1]] - ex[path[i]])) == 0)
        continue;
