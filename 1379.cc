...
double judge(point t)  //评价函数，得到点t的评价值val
{
  double len;
  len = 1LL << 45;
  for (int i = 0; i < n; i++) len = min(len, dis(t, p[i]));
  return len;
}
double Rand() {
...
void solve(double D) {
  ...
  May[3] = point(X, 0);
  // 4个顶点的可能行较大，所以特殊构造
  for (int i = 4; i < NUM; i++) May[i] = Rand_point(May[0], May[1]);  //步骤2
  while (D > 0.01)                                                    //步骤 3
  {
    for (int i = 0; i < NUM; i++)
      for (int j = 0; j < NUM; j++) {
        point tmp =
            Rand_point(point(max(0.0, May[i].x - D), max(0.0, May[i].y - D)),
  ...
  ans.val = 0;
  for (int i = 0; i < NUM; i++)
    if (May[i].val > ans.val) ans = May[i];
