...
Segment pack(const Segment &seg)  //进行打包
{
  ...
  if (seg.l * seg.r < seg.l + digNum + 2)  //+2是因为括号有两个字符
  {
    ...
    ret.l = seg.l * seg.r;
    strcpy(ret.s, seg.s);
    for (int i = 1; i < seg.r; i++) strcat(ret.s, seg.s);
  } else {
    ret.r = 1;
...
int linklen(const Segment &seg1, const Segment &seg2,
            int &linkr)  //返回将两个区间合并后的最小长度
{
  int len;
  if (seg1.l == seg2.l &&
      0 == strcmp(seg1.s, seg2.s))  //基本模式串一样则只需将重复次数相加
  {
    linkr = seg1.r + seg2.r;
    len = seg1.l;
  } else {
