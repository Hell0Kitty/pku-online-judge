...
void adde(int from, int to, int xia, int shang) {
  e[yong].u = from, e[yong].v = to, e[yong].f = shang - xia, e[yong].b = xia,
  e[yong].c = shang;
  e[yong].next = head[from], head[from] = yong++;
  e[yong].u = to, e[yong].v = from, e[yong].f = 0, e[yong].b = xia,
  e[yong].c = shang;
...
int max_f(int s, int t) {
  ...
  for (num[s] = t + 1; d[s] < t + 1;) f += sap_gap(s, inf, s, t);
  return f;
}
int main() {
