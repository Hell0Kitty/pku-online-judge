...
int main() {
  ...
  while (cin >> bex >> bey && (bex || bey)) {
    ...
    for (int i = 0; i < 3; i++) {
      cin >> x1 >> y1 >> x2 >> y2;
      if (x1 == x2)
        for (int st = min(y1, y2) + 1; st <= max(y1, y2); st++)
          hold[x1][st][x1 + 1][st] = hold[x1 + 1][st][x1][st] = 1;
      else
        for (int st = min(x1, x2) + 1; st <= max(x1, x2); st++)
          hold[st][y1][st][y1 + 1] = hold[st][y1 + 1][st][y1] = 1;
    }
    queue<node> que;
    node start;
