...
struct Interval {
  ...
  int value;
  Interval() {}
  Interval(int start, int end, int pos, int value)
      : start(start), end(end), pos(pos), value(value) {}
...
void recount(Node *p) {
  ...
  if (p->pleft->to_right != p->pright->to_left) p->edge_num++;
}
...
long long work() {
  ...
  for (int i = 0; i < interval_num; i++) {
    ...
    if (tree->to_left) line_num++;
    if (tree->to_right) line_num++;
    if (i != interval_num - 1)
