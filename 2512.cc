...
bool Avail(int iRow, int iCol) {
  return iRow >= 0 && iRow < 8 && iCol >= 0 && iCol < 8;
}

void FillDiagonal(int iRow, int iCol) {
  int i, j;
  for (j = 0; j < 4; j++)
    for (i = 1; Avail(iRow + i * DIAGONAL[j][0], iCol + i * DIAGONAL[j][1]);
         i++) {
...
void FillBeeline(int iRow, int iCol) {
  int i, j;
  for (j = 0; j < 4; j++)
    for (i = 1; Avail(iRow + i * BEELINE[j][0], iCol + i * BEELINE[j][1]);
         i++) {
...
int main() {
  char cLine[100], cDes[8][10], cBoard[8][10];
  int iPos, i, j, k, iCount, iHead, iEnd;
  while (gets(cLine)) {
    iHead = iEnd = 0;
    for (i = 0; i < 8; i++) {
      for (iEnd = iHead + 1; cLine[iEnd] != '/' && cLine[iEnd] != '\0'; iEnd++)
        ;
      cLine[iEnd] = '\0';
      strcpy(cDes[i], cLine + iHead);
      iHead = iEnd + 1;
    }
    memset(bSafe, true, sizeof(bSafe));
    memset(bOccupy, false, sizeof(bOccupy));
    ...
    for (i = 0; i < 8; i++)
      for (j = 0; j < 8; j++) {
        ...
        else if (tolower(cBoard[i][j]) == 'b')
          FillDiagonal(i, j);
        else if (tolower(cBoard[i][j]) == 'r')
          FillBeeline(i, j);
    ...
    for (i = 0; i < 8; i++)
      for (j = 0; j < 8; j++)
        if (bSafe[i][j] && !bOccupy[i][j]) iCount++;
    printf("%d\n", iCount);
  }
}
