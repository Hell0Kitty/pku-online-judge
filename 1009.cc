  ...
  while (cin >> width && width) {
    ...
    for (int p = 0; p <= PairNum; p++) {
      ...
      for (int i = row - 1; i <= row + 1;
           i++)  //枚举(row,col)周围及其自身共9个点(x,y)
        for (int j = col - 1; j <= col + 1; j++) {
          int tpos = i * width + j;  //得到(x,y)的顺序位置

          if (i < 0 || j < 0 || j >= width || tpos >= total) continue;

          OutMap[k].pos = tpos + 1;
          OutMap[k++].code =
              GetCode(tpos + 1);  //对发生变化的像素点的附近8个点编码
        }
    ...
    Pix temp = OutMap[0];
    for (int i = 0; i < k; i++) {
      if (temp.code == OutMap[i].code) continue;
  ...
  for (int i = row - 1; i <= row + 1; i++)
    for (int j = col - 1; j <= col + 1; j++) {
      ...
      if (i < 0 || j < 0 || j >= width || tpos >= total ||
          tpos == pos - 1)  // tpos==pos-1为中心的像素点，即当前待编码的点
        continue;
