...
void dfs(Pos sur, int pre) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (temp.i < 0 || temp.j < 0 || temp.i >= n || temp.j > temp.i) continue;

    if (mark[temp.i][temp.j] == true) continue;
    if (map[temp.i][temp.j] != pre && pre != 0) continue;
    if (map[temp.i][temp.j] == 0) continue;
    ...
    for (i = 0; i < n; i++) {
      for (j = 0; j <= i; j++) {
        if (map[i][j] == 0) {
          ...
          for (ii = 0; ii < n; ii++) {
            for (jj = 0; jj <= ii; jj++) {
              if (map[ii][jj] == 0 || mark[ii][jj] == true) continue;
              if (map[ii][jj] == num)
                ans_temp--;
              else
                ans_temp++;
