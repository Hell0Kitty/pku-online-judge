...
{
    return (P[l].x - P[s].x)*(P[r].y - P[s].y) - (P[r].x - P[s].x)*(P[l].y - P[s].y);
}
double cala(int cc,int nn,struct POINT *P,double h)
{
    ...
    for (int i = s - 1; i > 0; --i)
    {
        ...
        {
            u1 = (h - P[i].y)*(P[i].x - P[i + 1].x)/(P[i].y - P[i + 1].y) + P[i].x;
            a += (u1 - P[s].x)*(P[i + 1].y - P[s].y) - (P[i + 1].x - P[s].x)*(h - P[s].y);
            break;
        }
    }
    for ( int i = s + 1; i <= nn; ++i)
    {
        ...
        {
            u2 = (h - P[i].y)*(P[i - 1].x - P[i].x)/(P[i - 1].y - P[i].y) + P[i].x;
            a += (P[i - 1].x - P[s].x)*(h - P[s].y) - (u2 - P[s].x)*(P[i - 1].y - P[s].y);
            break;
        }else break;
    }
