...
struct pt {
    ...
    pt rotate(double ang) const { return pt(x*cos(ang)-y*sin(ang), x*sin(ang)+y*cos(ang)); }

    double len() const { return sqrt(x*x + y*y); }
    pt trunc(double s) const { double l = len(); return pt(x*s/l, y*s/l); }
...
double heightL(double x, double y) {
    return x * sqrt_3 + y;
}
double heightR(double x, double y) {
    return -x * sqrt_3 + y;
}
int main() {
    int test;
    ...
    for (int cas = 1; cas <= test; ++cas) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            left[i] = -1; right[i] = -1;
            left_dis[i] = 1e10; right_dis[i] = 1e10;
            ...
            if (left[i] != -1)
                left_dis[i] = heightL(tb[i]-b[i], o.y) - heightL(ta[left[i]]+a[left[i]], 0);
            if (right[i] != -1)
                right_dis[i] = heightR(tb[i]+b[i], o.y) - heightR(ta[right[i]]-a[right[i]], 0);
        ...
        for (int i = 0; i < n; ++i) {
            if (left_dis[i] < min_left) 
                min_left = left_dis[i];
            if (right_dis[i] < min_right) 
                min_right = right_dis[i];
        }
