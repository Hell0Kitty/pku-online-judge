...
segtree t;
void init(){
    t.build(); pt.resize(0); qy.resize(0);
...
void solve(){
    ...
    for (int i = 0; i < qy.size(); i++){
        ...
        n3[qy[i].i] = a3; n2[qy[i].i] = t.cnt[1] - a3;
    }
    t.build(); reverse(pt.begin(), pt.end()); reverse(qy.begin(), qy.end());
    pos = 0;
    for (int i = 0; i < qy.size(); i++){
        while(pt[pos].x > qy[i].x && pos < pt.size()){
            t.update(pt[pos].y, 0, N, 1);
        ...
        n4[qy[i].i] = a4; n1[qy[i].i] = t.cnt[1] - a4;
    }
    for (int i = 0; i < m; i++){
