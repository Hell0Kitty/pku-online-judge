...
void bfs(
    int fir_r,
    int fir_c) {  //  枚举顶点，用bfs构造顶点(fir_r,fir_c)d的所有边，加入邻接矩阵。
  int i, j;
  for (i = 1; i <= row; i++)
    for (j = 1; j <= col; j++) vis[i][j] = false;
  vis[fir_r][fir_c] = true;
  que[0].r = fir_r;
  que[0].c = fir_c;
  int num = n - 1, step = 0, head = 0,
      tail = 1;  // num表示未加入邻接矩阵的个数，step表示步数。
  while (tail - head > 0) {
    step++;
    int count = tail - head;
    while (count--) {
