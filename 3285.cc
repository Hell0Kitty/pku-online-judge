...
void solve(double D) {
  for (int i = 0; i < NUM; i++) May[i] = Rand_point(e1, e2);  //步骤2
  while (D > 0.001)                                           //步骤 3
  {
    for (int i = 0; i < NUM; i++)
      for (int j = 0; j < NUM; j++) {
        point tmp =
            Rand_point(point(max(e1.x, May[i].x - D), max(e1.y, May[i].y - D)),
                       point(min(e2.x, May[i].x + D), min(e2.y, May[i].y + D)));
  ...
  ans.val = inf;
  for (int i = 0; i < NUM; i++)
    if (ans.val > May[i].val) ans = May[i];
  if (ans.val < eps)
    printf("%.2f %.2f\n", ans.x, ans.y);
  else
    puts("No solution");
...
int main() {
  ...
  while (1) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      e2.x = max(p[i].x, e2.x);
      e2.y = max(p[i].y, e2.y);
      if (dd(p[i].x, 0.0) && dd(p[i].y, 0.0) && dd(p[i].val, 0.0)) num++;
    }
    if (num == 3) break;
