...
double solve() {
  if (xan == xbn && yan == ybn) return dis(xa, ya, xb, yb);
  int Max = max(abs(xbn - xan), abs(ybn - yan));
  return dis(xa, ya, xan * wd, yan * hd) + dis(xb, yb, xbn * wd, ybn * hd) +
         Max * sin(pi / 3) * 2 * l;
}

int main() {
  while (~scanf("%lf%lf%lf%lf%lf", &l, &xa, &ya, &xb, &yb) && l || xa || ya ||
         xb || yb) {
    init();
    printf("%.3lf\n", solve());
  }
