...
int between(point a, point b, point c) { return dblcmp(dot(a, b, c)); }
int seg_cross(point a, point b, point c, point d) {
  ...
  if (d1 == 0 && between(c, a, b) <= 0 || d2 == 0 && between(d, a, b) <= 0 ||
      d3 == 0 && between(a, c, d) <= 0 || d4 == 0 && between(b, c, d) <= 0)
    return 1;
  return 0;
}
...
bool cmp(node a, node b) { return a.no[0] < b.no[0]; }
...
void input() {
  ...
  if (c == 'p') {
    ...
    for (i = 1; i <= 3; i += 2)
      scanf(" (%lf,%lf)", &a[num].p[i].x, &a[num].p[i].y);
    a[num].p[2].x =
        (a[num].p[1].x + a[num].p[3].x + a[num].p[3].y - a[num].p[1].y) / 2.0;
    a[num].p[4].x =
        (a[num].p[1].x + a[num].p[3].x - a[num].p[3].y + a[num].p[1].y) / 2.0;
    a[num].p[2].y =
        (a[num].p[1].x - a[num].p[3].x + a[num].p[1].y + a[num].p[3].y) / 2.0;
    a[num].p[4].y =
        (a[num].p[3].x - a[num].p[1].x + a[num].p[1].y + a[num].p[3].y) / 2.0;
  ...
  while (scanf("%s", a[num].no) != EOF) {
    ...
    for (i = 0; i < num; i++) {
      ...
      if (len == 0)
        ...
        for (j = 0; j < len - 1; j++) printf("%c, ", a[v[i][j]].no[0]);
        printf("and %c\n", a[v[i][len - 1]].no[0]);
      }
    }
    puts("");
