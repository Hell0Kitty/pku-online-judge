...
double angle(Vector &v, City &c) {
  return acos((v.x * c.x + v.y * c.y + v.z * c.z) /
              sqrt(square(v.x) + square(v.y) + square(v.z)) /
              sqrt(square(c.x) + square(c.y) + square(c.z)));
}
...
int main() {
  ...
  while (scanf("%s", c[index].name) && strcmp(c[index].name, "#") != 0) {
    ...
    c[index].x = Radius * cos(radian(c[index].latitude)) *
                 sin(radian(c[index].longitude));
    c[index].y = Radius * cos(radian(c[index].latitude)) *
                 cos(radian(c[index].longitude));
  ...
  while (scanf("%s", A.name) && strcmp(A.name, "#")) {
    ...
    City *a, *b, *cen;
    a = find(c, c + index, A), b = find(c, c + index, B),
    cen = find(c, c + index, C);
