...
void compute() {
  for (int i = 0; i < (1 << n); ++i)
    for (int j = 0; j < n; ++j)
      for (int k = 0; k < n; ++k) dp[i][j][k].first = 256;
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      if (i == j) continue;
  ...
  for (int i = 0; i < (1 << n); ++i) {
    for (int x = 0; x < n; ++x) {
      if (((i >> x) & 1) == 0) continue;
      for (int y = 0; y < n; ++y) {
        if (y == x || ((i >> y) & 1) == 0) continue;
        if (dp[i][x][y].first == 256) continue;
        ...
        for (int j = 0; j < n; ++j) {
          ...
          if (sgn((p[j] - p[x]) * (p[y] - p[x])) == 0) {
            if (sgn((p[j] - p[y]) ^ (p[x] - p[y])) < 0) {
              res = min(res,
                        make_pair(cur.first, cur.second + (p[j] - p[y]).len()));
            } else {
              res = min(res, make_pair(cur.first + 1,
                                       cur.second + (p[j] - p[y]).len()));
            }
          } else {
            res = min(res, make_pair(cur.first + 1,
                                     cur.second + (p[j] - p[y]).len()));
        ...
        for (int j = 0; j < n; ++j) {
          ...
          for (int k = 0; k < n; ++k) {
            ...
            if (get_intersection(p[x], p[y], p[j], p[k], c)) {
              if (sgn((p[x] - p[y]) ^ (c - p[y])) < 0 &&
                  sgn((p[k] - p[j]) ^ (c - p[j])) < 0) {
                res = min(res, make_pair(cur.first + 1, cur.second +
                                                            (c - p[y]).len() +
                                                            (c - p[k]).len()));
              }
            }
  ...
  ans.first = 256;
  for (int i = 0; i < n; ++i)
    for (int j = 0; j < n; ++j) ans = min(ans, dp[(1 << n) - 1][i][j]);
}
