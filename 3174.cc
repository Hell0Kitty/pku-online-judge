...
__inline void update(int start, int end) {
  ...
  for (i = start; i <= end; i++) a->id[a->len++] = slash[i].to;
...
__inline void bsort(struct seg_node *arr, int len) {
  ...
  for (i = 0; i < len - 1; i++)
    for (j = i; j < len - 1; j++)
      if (cmp_seg(&arr[j], &arr[j + 1]) > 0) swap_seg(&arr[j], &arr[j + 1]);
}
...
__inline int calc(int i) {
  ...
  for (from = slash[i].from; slash[i].from == from; i++) {
    start = i;
    while (i + 1 < slash_cnt && slash[i + 1].from == from &&
  ...
  if (cnt) {
    bsort(arr, cnt);
    for (j = 0; j < cnt; j++) update(arr[j].start, arr[j].end);
  }
...
int main() {
  ...
  slash_cnt = 0;
  for (i = 1; i <= N; i++)
    for (j = i + 1; j <= N; j++) add_slash(i, j);
  qsort(slash, slash_cnt, sizeof(slash[0]), cmp_slash);
  for (i = 0; i < slash_cnt; i++) i = calc(i);

  cnt = 0;
  for (i = 0; i < ans_cnt; i++)
    cnt += (ans[i].len) * (ans[i].len - 1) * (ans[i].len - 2) / 6;
  printf("%d\n", cnt);
  for (i = 0; i < ans_cnt; i++) dump_ans(&ans[i]);
