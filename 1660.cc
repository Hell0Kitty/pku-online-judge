...
bool ROPE::isSelfKnoted() {
  ...
  while (tmp->next != begin) {
    ...
    while (tmp2->nodeNo != -no && (sign * tmp2->nodeNo > 0)) tmp2 = tmp2->next;
    if (tmp2->nodeNo == -no) {
      begin = tmp;
      return true;
    }
...
bool ROPE::deleteSelfNotedNodes() {
  ...
  for (int i = 0; i < s; i++) deleteNode(nodes[i]);
  for (int i = 0; i < s; i++) deleteNode(-nodes[i]);
  return true;
}
...
bool ROPE::isPairKnoted(int &b1, int &e1, int &dir) {
  ...
  while (tmp->next != begin) {
    ...
    NODE *tmp1, *tmp2;
    tmp2 = tmp->next;
    while ((tmp->nodeNo * tmp2->nodeNo < 0) && tmp2 != tmp) tmp2 = tmp2->next;
    if (tmp2 != tmp) e1 = tmp2->nodeNo;
    tmp1 = findNode(-b1);
    tmp2 = findNode(-e1);
    while (tmp1->nodeNo * tmp2->nodeNo > 0 && tmp1 != tmp2) tmp1 = tmp1->next;
    if (tmp1 == tmp2) {
      dir = +1;
      return true;
    }
