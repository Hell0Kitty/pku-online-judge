...
int main() {
  ...
  for (i = 0; i <= MAX_N + 1; i++) expV[i] = pow(double(2), i);

  cin >> caseN;
  for (i = 1; i <= caseN; i++) {
    init();
    cin >> minCost >> maxCost >> fNum >> cNum;
    for (j = 0; j < fNum; j++) cin >> fCost[j];
    for (j = 0; j < cNum; j++) {
      cin >> cReq[j][0];
      int total = 0;
    ...
    for (curState = 0; curState <= expV[fNum] - 1; curState++) {
      ...
      if (!(curCost >= minCost && curCost <= maxCost)) continue;
      for (j = 0; j < cNum; j++) {
      ...
      if (update) {
        ...
        maxSales = curSales;
        maxExp = curCost;
        for (j = 0; j < cNum; j++) cServerd[j] = cSAssit[j];
      }
    }
