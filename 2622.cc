...
struct pt {
  ...
  bool operator<(const pt &p1) const {
    return y < p1.y - eps || fabs(y - p1.y) < eps && x < p1.x;
  }
...
void make_ch(pt *p, pt *f, int n, int &top) {
  ...
  for (int i = 0; i < 2 * n - 1; i++) {
    int j = (i < n) ? i : 2 * (n - 1) - i;
    while (top > 1 && cpr(f[top - 2], f[top - 1], p[j]) < -eps) top--;
    f[top++] = p[j];
  }
  top--;
}
...
void init() {
  ...
  for (i = 0; i < n; i++) scanf("%lf%lf", &ori[i].x, &ori[i].y);
  make_ch(ori, dest, n, num);

  for (i = 0; i < num; i++) {
    if (dcmp(dest[i].y - tp) == 1) {
      tp = dest[i].y;
    }
    if (dcmp(dn - dest[i].y) == 1) {
      dn = dest[i].y;
    }
    if (dcmp(dest[i].x - rt) == 1) {
      rt = dest[i].x;
    }
    if (dcmp(lf - dest[i].x) == 1) {
      lf = dest[i].x;
    }
  }
  left_top.x = lf;
...
void Solve() {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (dcmp(pld - cross.y) == 1) {
      pld = cross.y;
    }
    cross = its(a, b, left_down, right_down);

    if (dcmp(pdl - cross.x) == 1) {
      pdl = cross.x;
    }
    cross = its(a, b, left_top, right_top);

    if (dcmp(cross.x - ptr) == 1) {
      ptr = cross.x;
    }
    cross = its(a, b, right_top, right_down);

    if (dcmp(cross.y - prt) == 1) {
      prt = cross.y;
    }
    b = a + (pt){1, 1};  //鍙充笂绾�
    cross = its(a, b, left_top, left_down);

    if (dcmp(cross.y - plt) == 1) {
      plt = cross.y;
    }
    cross = its(a, b, left_top, right_top);

    if (dcmp(ptl - cross.x) == 1) {
      ptl = cross.x;
    }
    cross = its(a, b, left_down, right_down);

    if (dcmp(cross.x - pdr) == 1) {
      pdr = cross.x;
    }
    cross = its(a, b, right_top, right_down);

    if (dcmp(prd - cross.y) == 1) {
      prd = cross.y;
    }
  }
  res[0] = (pt){lf, plt};
