...
int DP_MAX(int i, int j)  // DP求区间最大值
{
  ...
  for (int k = i; k < j; k++) {
    ...
    int rr = DP_MAX(k + 1, j);
    if (!op[k])
      dp_max[u] = MAX(dp_max[u], ll + rr);
    else
      dp_max[u] = MAX(dp_max[u], MAX(ll * rr, MAX(l * r, MAX(l * rr, r * ll))));
  }
  return dp_max[u];
}
...
int DP_MIN(int i, int j)  // DP求区间最小值
{
  ...
  for (int k = i; k < j; k++) {
    ...
    int rr = DP_MAX(k + 1, j);
    if (!op[k])
      dp_min[u] = MIN(dp_min[u], l + r);
    else
      dp_min[u] = MIN(dp_min[u], MIN(ll * rr, MIN(l * r, MIN(l * rr, r * ll))));
  }
  return dp_min[u];
}
