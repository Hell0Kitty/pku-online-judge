...
int main() {
  while (true) {
    ...
    while (true) {
      ...
      if (scanf("%s", toppingConstraintString) == EOF) return 0;
      if (strcmp(toppingConstraintString, TOPPING_CONSTRAINT_END_STRING) == 0)
        break;
      includeConstraints[toppingConstraintNum] =
          omitConstraints[toppingConstraintNum] = 0;
      for (int toppingRequestID = 0;
           toppingConstraintString[2 * toppingRequestID] != ';';
           toppingRequestID++) {
    ...
    for (int topping = 0; topping < (1 << TOPPING_TYPE_NUM); topping++) {
      bool satisfied = true;
      for (int toppingConstraintID = 0;
           toppingConstraintID < toppingConstraintNum; toppingConstraintID++) {
        if (topping & includeConstraints[toppingConstraintID]) continue;
        if ((topping & omitConstraints[toppingConstraintID]) ^
            omitConstraints[toppingConstraintID])
          continue;
        satisfied = false;
        break;
      }
      if (satisfied) {
        ...
        for (int toppingID = 0; toppingID < TOPPING_TYPE_NUM; toppingID++)
          if (topping & (1 << toppingID)) printf("%c", 'A' + toppingID);
        printf("\n");
        break;
      }
    }
    if (!anyoneSatisfied) printf("No pizza can satisfy these requests.\n");
  }
  return 0;
}
