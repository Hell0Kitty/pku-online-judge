    ...
    if (n == 1) {
      ...
      for (ls_index = 2; ls_index < n; ls_index++) {
        while (stack_size > 1) {
          ...
          if (is_point_fit_line(ls[top], intersection(ls[ls_index], ls[prev])))
            --stack_size;
          else {
            break;
          }
      ...
      for (st_index = 0; st_index < m; st_index++) {
        if (st[st_index].a * ls[0].b > st[st_index].b * ls[0].a)
      ...
      while (cur < stack_size && st_index < m) {
        ...
        while (cur < stack_size) {
          ls_index = stack[cur];
          LL det =
              st[st_index].a * ls[ls_index].b - st[st_index].b * ls[ls_index].a;
          if (det >= 0) {
            ans[st[st_index].c] =
                st[st_index].a * p[cur].x + st[st_index].b * p[cur].y;
            break;
          } else {
            cur++;
            continue;
