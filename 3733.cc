...
struct poly {
  int n;
  cpoint cp[M];
  cpoint &operator[](int k) { return cp[k]; }
} ply[N], ply2[N];
...
bool cmp(const cpoint &u, const cpoint &v) {
  return dcmp(fabs(u.x - bp.x) + fabs(u.y - bp.y) - fabs(v.x - bp.x) -
              fabs(v.y - bp.y)) < 0;
}
...
bool check(cpoint u, cpoint v, poly ply[], int id) {
  ...
  for (i = 0; i < ply[id].n; ++i) {
    ...
    if (PointOnSegment(cp, *p1, *p2)) {
      ...
      if (chk == 0) chk = dcmp(u.y - v.y) * dcmp(p1->y - p2->y);
      if (chk == -1) return 0;
      return vis[id][i];
    }
...
double PolyUnion(poly ply[], int &n) {
  double ans = 0;
  int i, j, k, vi, vj, tn = n;
  n = 0;
  cpoint cp, p1, p2;
  memset(vis, 0, sizeof(vis));
  for (i = 0; i < tn; ++i) {
    if (dcmp(cross(ply[i][0], ply[i][1], ply[i][2])) <= 0) continue;
  ...
  for (i = 0; i < n; ++i) {
    for (vi = 0; vi < ply[i].n; ++vi) {
      ...
      for (k = 1; k < tn; ++k) {
        for (j = 0; j < n; ++j)
          if (i != j)
            if (check(tp[k - 1], tp[k], ply, j)) break;
        if (j == n) ans += tp[k - 1].x * tp[k].y - tp[k - 1].y * tp[k].x;
      }
      vis[i][vi] = 1;
    }
  }
...
double dissqr(cpoint u, cpoint v) { return sqr(u.x - v.x) + sqr(u.y - v.y); }
...
void solve(int cas) {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    if (dcmp(cross(a[0], a[1], a[2])) < 0) reverse(a, a + 4);
    a[4] = a[0];
    int tn = 0;
    for (int j = 0; j < 4; ++j) {
      tmp[tn++] = a[j];
