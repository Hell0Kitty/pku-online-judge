...
struct BigNum {
  ...
  BigNum operator*(BigNum temp) {
    ...
    for (i = a[0]; i >= 1; i--)
      for (j = temp.a[0]; j >= 1; j--) ans.a[i + j - 1] += a[i] * temp.a[j];
    ans.a[0] = 0;
    for (i = p; i >= 1; i--) {
      ans.a[i - 1] += ans.a[i] / mod;
      ans.a[i] %= mod;
  ...
  int operator>(BigNum temp) {
    ...
    if (a[0] < temp.a[0]) return 0;
    for (int i = 1; i <= a[0]; i++)
      if (a[i] != temp.a[i]) return a[i] > temp.a[i];
    return 0;
  }
...
void print(BigNum a) {
  ...
  printf("%d ", j % 10);
  for (i = V.size() - 1; i >= 10; i--) printf("%d", V[i]);
  putchar('.');
  for (i = 9; i >= 0; i--) printf("%d", V[i]);
  puts("");
}
