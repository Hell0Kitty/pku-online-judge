...
void print(statusN *cur) {
  ...
  int bitT = cur->bit, bitF = cur->next->bit, room2 = cur->rNum,
      room1 = cur->next->rNum;
  if (bitT == bitF)
...
bool solve() {
  ...
  while (!Q.empty()) {
    ...
    for (i = 0; i < doorNum; i++) {
      int opp;
      if ((opp = door[i].getOpp(room)) != -1 && (bit & exp[opp]) > 0 &&
          (!v[opp][bit])) {
        v[opp][bit] = 1;
...
bool compare(controlN c1, controlN c2) {
  if (c1.hNum < c2.hNum)
    ...
    if (c1.lNum <= c2.lNum) return true;
    return false;
  }
  return false;
}
...
int main() {
  int i, j, f, t;
  exp[0] = 1;
  for (i = 1; i <= MAX_H; i++) exp[i] = exp[i - 1] << 1;
  ...
  int caseN = 0;
  while (cin >> roomNum >> doorNum >> switNum &&
         (roomNum + doorNum + switNum) != 0) {
    caseN++;
    for (i = 0; i < doorNum; i++) {
      cin >> f >> t;
      doorN cur = {f - 1, t - 1};
