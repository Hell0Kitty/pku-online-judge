...
void change_color(int* arr, int i) {
  ...
  int x = i / 4;
  int y = i % 4;
  if (y < 3) arr[i + 1] = !(arr[i + 1]);
  if (y > 0) arr[i - 1] = !(arr[i - 1]);
  if (x > 0) arr[i - 4] = !(arr[i - 4]);
  if (x < 3) arr[i + 4] = !(arr[i + 4]);
}
void combine(int* arr, int len, int* result, int count, const int NUM,
             int* last) {
  int i;
  for (i = len; i >= count; i--) {
    result[count - 1] = i - 1;
    if (count > 1)
      ...
      for (j = 0; j < 16; j++) new_arr[j] = arr[j];
...
int main() {
  ...
  if (all_white_or_black(bits, 16))
    ...
    int i;
    for (i = 0; i < 16; i++) new_bits[i] = bits[i];
