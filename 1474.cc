...
struct segment
  ...
  inline void setsegment(point a, point b) {
    p1 = a;
    p2 = b;
  }
...
struct line
  ...
  {
    if (fabs(x.a - y.a) < eps && fabs(x.b - y.b) < eps && fabs(x.c - y.c) < eps)
      return true;
...

{
  if ((max(l1.p1.x, l1.p2.x) >= min(l2.p1.x, l2.p2.x)) &&
      ...
      fabs((xmult(l2.p1, l1.p1, l1.p2) * xmult(l2.p2, l1.p1, l1.p2)) <=
           eps))  //跨立实验
...

  {
...

    {
...

      {
        ...
        if (xmult(pvec[i], s.p2, s.p1) * xmult(pvec[(i + 1) % sz], s.p2, s.p1) <
            0)
...

{
...

  {
    ...
    if (poly.sz > 0)
      print("Surveillance is possible.");

    else
      print("Surveillance is impossible.");
