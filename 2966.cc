...
bool Point_In_Segment(Point a, Point b, Point t) {
  return (t.x <= max(a.x, b.x) && t.x >= min(a.x, b.x) &&
          t.y <= max(a.y, b.y) && t.y >= min(a.y, b.y) &&
...
bool Linelenth_In_Polygon(Line l1) {
  ...
  for (i = 0; i < n; i++) {
    if ((x_mult(p[i], l1.a, l1.b) * x_mult(p[(i + 1) % n], l1.a, l1.b) <= EP) &&
        (x_mult(l1.a, p[i], p[(i + 1) % n]) *
             x_mult(l1.b, p[i], p[(i + 1) % n]) <=
         EP)) {
  ...
  for (i = 0; i < cnt - 1; i++) {
    ...
    int flag = 0;
    for (j = 0; j < n; j++) {
      if (Point_In_Segment(p[j], p[(j + 1) % n], mid)) flag = 1;
    }
    if (flag) continue;
    if (Point_In_Polygon(mid)) ans += dist(tp[i], tp[i + 1]);
