...
bool Huffman::Solve() {
  while (tree.size() >= digitNum) {
    ...
    for (int i = 0; i < digitNum; i++) {
      if (combine->value > tree[i]->value) combine->value = tree[i]->value;
      combine->frequency += tree[i]->frequency;
      if (i + 1 < digitNum) tree[i]->sibling = tree[i + 1];
    }
...
bool Huffman::Coding(Node* root, string newCode) {
  root->code = newCode;
  if (root->child != NULL) Coding(root->child, newCode + "0");
