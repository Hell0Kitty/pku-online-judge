...
int cmp(int a, int b) {
  return findfather(a) == findfather(b) ? (tot[a] > tot[b])
                                        : (findfather(a) < findfather(b));
}
...
int main() {
  ...
  while (scanf("%d%d", &n, &m), n) {
    ...
    for (i = 0; i < n;) {
      ...
      if (!search(++i)) break;
      while (i < n && father[list[i]] == father[list[i - 1]]) i++;
    }
    if (i < n)
      puts("The agents cannot be split");
    else {
      for (i = 0; i < n - 1; i++) printf("%d ", inset[i]);
      printf("%d\n", inset[n - 1]);
    }
  }
  return 0;
