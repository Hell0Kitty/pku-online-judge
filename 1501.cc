...
int inline round(int para){
	if( para < 0 ) return dim + para;
	else if( para >= dim) return para - dim; 
	return para;
}
...
void proc(string& word){
	  ...
	  for (int j = 0; j < dim; ++j)
	{
		if( matrix[i][j] == word[0]){
			//four direction;
			// right
		//	cout << "[dbg] match point start at: " <<  i  << " " << j << endl;
			bool matched = true; 
			int t = 0;
		...
		//	cout << "[dbg] now go right" << endl;
			for (; t < word.size(); ++t)
			{
				if(word[t] == matrix[i][round(j+t)]) continue; 
		...
		//	cout << "[dbg] now go down" << endl;
			...
			//		cout << "[dbg] current coordinate: " << round(i+t) << " " << j << endl;
					continue;
				} 
				else { matched = false; break; }
			}
		...
		//	cout << "[dbg] now go up" << endl;
			for ( t = 0; t < word.size(); ++t)
			{
				if(word[t] == matrix[round(i -t)][j]) continue; 
				else { matched = false; break; }
			}
			if( matched ) {
				global_found = true; 
