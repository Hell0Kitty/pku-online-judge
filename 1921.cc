...
double dist(const Point &p1, const Point &p2) { return (p1 - p2).length(); }
...
struct Poly {
  vector<int> id;
  void add(int i) { id.push_back(i); }
  Point &operator[](int i) const { return p_vec[id[i]]; }
};
...
int intersection(int id1, int id2, const Point &p1, const Point &p2) {
  ...
  if (it == edge_map.end()) {
    ...
    edge_map[make_pair(id1, id2)] = edge_map[make_pair(id1, id2)] =
        p_vec.size() - 1;
    return p_vec.size() - 1;
  } else
    return it->second;
...
void fold(const Point &p1, const Point &p2) {
  ...
  edge_map.clear();
  pol_buf.clear();
  for (int i = 0; i < int(pol_vec.size()); ++i) fold(p1, p2, pol_vec[i]);
  pol_vec = pol_buf;
  for (int i = 0; i < int(p_buf.size()); ++i) p_vec[i] = p_buf[i];
}
...
void dfs(vector<bool> &vis, int id, const Line &l) {
  ...
  for (int i = 0; i < int(pol.id.size() - 1); ++i) {
    if (sgn(cross(l.ed, pol[i], l.st)) == 0 &&
        sgn(cross(l.ed, pol[i + 1], l.st)) == 0)
      continue;
