...
void change(int p, int l, int r, long long x) {
  if (l & lt; = tr[p].l & amp; &amp; r & gt; = tr[p].r) {
    tr[p].dat += x;
    tr[p].add += x;
    return;
  }
  ...
  if (l & lt; = mid) change(p * 2, l, r, x);
  if (r & gt; mid) change(p * 2 + 1, l, r, x);
  tr[p].dat = maxl(tr[p * 2].dat, tr[p * 2 + 1].dat);
}
void make() {
  sort(r + 1, r + tot + 1);
  for (int i = 1; i & lt; = cnt; i++) {
...
int main() {
  while (scanf("%d%I64d%I64d", &amp; n, &amp; w, &amp; h) != EOF) {
    ...
    for (int i = 1; i & lt; = n; i++) {
      ...
      pos[++cnt].x = a, pos[cnt].y1 = b, pos[cnt].y2 = b + h - 1,
      pos[cnt].c = c;
      pos[++cnt].x = a + w, pos[cnt].y1 = b, pos[cnt].y2 = b + h - 1,
      pos[cnt].c = -c;
      r[++tot] = b, r[++tot] = b - 1, r[++tot] = b + 1, r[++tot] = b + h - 1,
      r[++tot] = b + h, r[++tot] = b + h + 1;
    }
    make();
    build(1, 1, mx + 10);
