...
const int N = 14, N1 = 1 << N;

int n, n1, a[N][N], dp[N1];

int main() {
  repc(rdi()) {
    scanf("%d", &n), n1 = 1 << n;
    forl(i, 0, n) forl(j, 0, n) scanf("%d", a[i] + j);
    rst(dp, 63), dp[0] = 0;
    forlc(u, 0, n1) {
      forl(i, 0, n) if (!bit(u, i)) {
        int v = u | 1 << i, cost = a[i][i];
        forl(j, 0, n) if (bit(u, j)) cost += a[i][j];
        updateMin(dp[v], dp[u] + cost);
      }
    }
    static int cas;
    printf("Scenario #%d:\n", ++cas);
