...
int main() {
  ...
  while (scanf("%s", &strs) && strs[0] != '-') {
    ...
    for (printf("%s ", strs), i = 0; strs[i] != 0 && ++loop[0][strs[i] - 48];
         i++)
      ;
    for (
        i = 0;
        i < 10 &&
            (loop[0][i] != 0 &&
                 (strcat(tmp, (sprintf(tmpInt, "%d", (int)loop[0][i]), tmpInt)),
                  strcat(tmp, (sprintf(tmpInt, "%d", i), tmpInt))),
             true);
        i++)
      ;
    ...
    for (j = 1; j < 15; j++) {
      ...
      for (i = 0, tmp[0] = 0;
           i < 10 && (loop[j][i] != 0 &&
                          (strcat(tmp, (sprintf(tmpInt, "%d", (int)loop[j][i]),
                                        tmpInt)),
                           strcat(tmp, (sprintf(tmpInt, "%d", i), tmpInt))),
                      true);
           i++)
