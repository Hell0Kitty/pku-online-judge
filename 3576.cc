...
const int MAX_N = 5000, MAX_L = 30, SON = 26, MAX_NODE_CNT = MAX_N * MAX_L + 1, SEED = 99991;
char pat[MAX_L + 1];
    ...
    while (scanf("%d", &n) != EOF && n) {
        ...
        for (int i = 0, j = 0; i < node_idx; i = j) {
            ...
            for (int k = i; k < j; k ++) {
                ...
                for (int s = 0; s < SON; s ++) {
                    int ss = node[seq[k]].son[s] >= 0 ? ds.get_root(node[seq[k]].son[s]) + 2 : 1;
            ...
            for (int k = i, l = i; k < j; k = l) {
                while (l < j && node[seq[k]].sc == node[seq[l]].sc && node[seq[k]].hash == node[seq[l]].hash) l ++;
                ans -= (l - k - 1);
