...
inline double cross(const point &p1, const point &p2, const point &q1,
                    const point &q2) {
...
inline bool cmp2(const point &a, const point &b)  //逆时针排序
{
  ...
  return cross(origin, a, origin, b) > 0 ||
         (cross(origin, a, origin, b) == 0 && fabs(a.x) < fabs(b.x));
}
...
inline void convex_hull(int &cnt, int n) {
  ...
  for (int i(2); i < n; ++i) {
    while (cnt >= 2 &&
           cross(convex[cnt - 2], convex[cnt - 1], convex[cnt - 1], data[i]) <=
               0)
      --cnt;
    convex[cnt++] = data[i];
  }
...
inline double rotating_calipers(int n) {
  ...
  for (int i(0); i < n; ++i) {
    ...
    while (j != i && k != i) {
      ...
      while (
          fabs(cross(convex[i], convex[j], convex[i], convex[k])) <
          fabs(cross(convex[i], convex[j], convex[i], convex[(k + 1) % n]))) {
        k = (k + 1) % n;
      }
      j = (j + 1) % n;
