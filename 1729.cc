...
struct status {
 public:
  ...
  int pre;
  int id;
  bool operator<(const status& r) const { return this->mindist < r.mindist; }
};
int fx[] = {0, -1, 0, 1};
int fy[] = {1, 0, -1, 0};
char move[] = "ENWS";
...
void BFS(int hx, int hy, int Hx, int Hy) {
  ...
  while (!que.empty()) {
    ...
    if (mp[y.hx][y.hy] == 's' && mp[y.Hx][y.Hy] == 'S') {
      ...
      stack<char> st1, st2;
      while (y.pre != -1) {
        if (y.moveH != '*') st1.push(y.moveH);
        if (y.moveh != '*') st2.push(y.moveh);
        y = way[y.pre];
      }
      while (!st1.empty()) {
        printf("%c", st1.top());
    ...
    for (i = 0; i < 4; i++) {
      ...
      int b = y.hy + fy[i];
      char moveh = move[i];
      if (mp[y.hx][y.hy] == 's') a = y.hx, b = y.hy, moveh = '*';
      if (a >= 0 && a < n && b >= 0 && b < n && mp[a][b] != '*' &&
        ...
        for (j = 0; j < 4; j++) {
          ...
          int d = y.Hy + fy[j];
          char moveH = move[j];
          if (mp[y.Hx][y.Hy] == 'S') c = y.Hx, d = y.Hy, moveH = '*';
          if (c >= 0 && c < n && d >= 0 && d < n && mp[c][d] != '*' &&
              mp[c][d] != 's' && mp[c][d] != 'h') {
            dis = dist(a, b, c, d);
