...
struct Edge {
  ...
  Edge(int u, int v, int cap, int f, int cost)
      : u(u), v(v), cap(cap), flow(f), cost(cost) {}
};
...
int main() {
  ...
  while (~scanf("%d%d%d", &n, &m, &k) && n + m + k) {
    for (int i = 1; i <= n; i++)
      for (int j = 1; j <= k; j++) scanf("%d", &ord[i][j]);
    for (int i = 1; i <= m; i++)
      for (int j = 1; j <= k; j++) scanf("%d", &sto[i][j]);
    int S = 0, T = n + m + 2;
    int cost = 0;
    for (int p = 1; p <= k; p++) {
      int sum = 0;
      ...
      for (int i = 1; i <= m; i++) net.AddEdge(S, i + n, sto[i][p], 0);
      for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++) {
          int x;
          scanf("%d", &x);
      ...
      if (~cost && net.Mincost(S, T, cost) < sum) cost = -1;
    }
    printf("%d\n", cost);
  }
  return 0;
