...
class Scope {
 ...
 public:
  int& operator[](const string& name) { return nameValueMap[name]; }
} global;
...
struct IfSuite : public Expression {
  ...
  void addClause(Expression* exp) { clause.push_back(exp); }
  virtual void execute() {
