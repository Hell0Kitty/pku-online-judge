...
using namespace std;
const int add = 200;
int max_s = 200, left_ = 1, right_ = 2, up = 4, down = 8, st[401][401], ps,
    b[2][401][401];
struct point {
  int x, y;
};
...
bool test(int state, int flag) { return (state & flag) == flag; }
...
void get_node() {
  ...
  for (s = 0; s < 2; s++) {
    for (i = -max_s; i <= max_s; i++) {
      ...
      for (j = -max_s; j <= max_s; j++) {
        ...
        if (ps == _out_) {
          int vv = test(st[x + add][y + add], nle),
              vp = test(st[x + add][y + add], ndo);
          ...
          int vv = test(st[x + add][y + add], nle),
              vp = test(st[x + add][y + add], nri);
          if (vv) {
            ps = _out_;
            out[x + add][y + add] = true;
          } else if (vp) {
            ps = _trace_;
            if (s == 0) tr[i + add][j + add] = true;
          }
        } else if (ps == _side2_) {
          int vv = test(st[x + add][y + add], nle),
              vp = test(st[x + add][y + add], nri);
        ...
        if (gao) {
          ...
          rr[s][node[s]].y = y;
          for (px = sx; px <= x; px++)
            for (py = sy; py <= y; py++) b[s][px + add][py + add] = node[s];
          node[s]++;
        }
        if (ps != _trace_) {
          sy = j + 1;
  ...
  for (i = 0; i < node[0]; i++)
    for (j = 0; j < node[1]; j++) mat[i][j] = false;
  for (i = -max_s; i <= max_s; i++)
    for (j = -max_s; j <= max_s; j++) {
  ...
  for (i = 0; i < node[0]; i++) {
    if (l[i] < 0 && dfs(i)) {
      for (j = 0; j < node[0]; j++) vl[j] = false;
      for (j = 0; j < node[1]; j++) vr[j] = false;
    }
  }
  ...
  for (i = 0; i < node[0]; i++) {
    if (vl[i]) draw(ll[0][i].x, ll[0][i].y, rr[0][i].x, rr[0][i].y);
  }
  for (i = 0; i < node[1]; i++) {
    if (!vr[i]) draw(ll[1][i].x, ll[1][i].y, rr[1][i].x, rr[1][i].y);
  }
  for (i = -max_s; i <= max_s; i++) {
    bool now_tr = false;
    for (j = -max_s; j <= max_s; j++) {
      ...
      v2 = (test(st[i + add][j + add], left_) &
            test(st[i + add][j + add], right_));
      if (tr[i + add][j + add] && !now_tr) {
        now_tr = true;
        sy = j;
      }
      if ((v1 || v2) && now_tr) {
        if (sy < j) draw(i, sy, i, j);
        now_tr = false;
      }
    }
    now_tr = false;
  ...
  for (i = -max_s; i <= max_s; i++)
    for (j = -max_s; j <= max_s; j++) {
      int v = (test(st[i + add][j + add], down) &
               test(st[i + add][j + add], left_));
      v &= !out[i + add][j + add];
      if (v) {
        for (p = i + 1; p <= max_s; p++) {
          v = (test(st[p + add][j + add], down) &
               test(st[p + add][j + add], right_));
          if (v) break;
        }
        if (p <= max_s) {
          rec[num_rec].x1 = i;
          rec[num_rec].x2 = p;
          for (p = j + 1; p <= max_s; p++) {
            v = (test(st[i + add][p + add], up) &
                 test(st[i + add][p + add], left_));
            if (v) break;
          }
          if (p <= max_s) {
            rec[num_rec].y1 = j;
