...
int main() {
  ...
  while (~scanf("%d", &n) && n > 0) {
    ...
    for (i = 2; i <= n; i++)
      for (j = i + 1; j <= n; j++)
        if (jiaodian(p[i], p[i - 1], p[j], p[j - 1], temp) &&
            Onsegment(temp, p[i], p[i - 1]) && Onsegment(temp, p[j], p[j - 1]))
          p2[++tot] = temp;
    for (i = 1; i <= n; i++) p2[++tot] = p[i];
    sort(p2 + 1, p2 + 1 + tot);
    ret = 1;
    for (i = 2; i <= tot; i++)
      if (!(p2[i] == p2[i - 1])) p2[++ret] = p2[i];
    tot = ret;
    ret2 = 0;
    for (i = 2; i <= n; i++)
      for (j = 1; j <= tot; j++)
        if (Onsegment(p2[j], p[i], p[i - 1])) ret2++;
