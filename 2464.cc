...
struct Point {
  int x, y;
  bool operator<(const Point &argu) const { return x < argu.x; }
} pp[MAXN];
...
int main() {
  ...
  while (scanf("%d", &n) && n) {
    for (int i = 0; i < n; i++)
      scanf("%d%d", &pp[i].x, &pp[i].y), ny[i] = pp[i].y;
    sort(ny, ny + n);

    int cnty = 0;
    for (int i = 0; i < n; i++)
      if (ny[cnty] != ny[i]) ny[++cnty] = ny[i];
    cnty++;
    for (int i = 0; i < n; i++)
      pp[i].y = lower_bound(ny, ny + cnty, pp[i].y) - ny + 1;
