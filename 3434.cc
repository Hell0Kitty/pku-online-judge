...
inline void setvisit(const cpoint &cp) { visit[cp.y][cp.x] = 1; }

inline bool visited(const cpoint &cp) { return visit[cp.y][cp.x]; }
  ...
  for (int i = 0; i < k; i++) {
    for (int j = 0; j < nsnake; j++) {
      ...
      if (array[j].head.y + my[pos] >= 0 && array[j].head.y + my[pos] < n &&
        ...
        if (array[j].head.y + my[right] >= 0 &&
            array[j].head.y + my[right] < n &&
            array[j].head.x + mx[right] >= 0 &&
            array[j].head.x + mx[right] < n &&
          ...
          array[j].body.pop();
          array[j].pos = right;
        } else if (array[j].head.y + my[left] >= 0 &&
                   array[j].head.y + my[left] < n &&
                   array[j].head.x + mx[left] >= 0 &&
                   array[j].head.y + mx[left] < n &&
