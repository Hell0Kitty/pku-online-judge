...
int main(){
	...
	while (cin>>n)
	{
		...
		while (cnt>1)
		{
			...
			int num=cnt;
			if(cnt&1) num--;
			Rep(i, vec.size()) if(del[vec[i].first]==0 && del[vec[i].second]==0 && play[vec[i].first]==0 && play[vec[i].second]==0)
			{
				if(in[vec[i].first]>=1 && in[vec[i].second]==1)
				{
					...
					num--;
					del[vec[i].second]=1;
					cout<<name[vec[i].first]<<" defeats "<<name[vec[i].second]<<endl;
				}
				else if(in[vec[i].first]==1 && in[vec[i].second]>=1)
				{
					...
					num--;
					del[vec[i].first]=1;
					cout<<name[vec[i].second]<<" defeats "<<name[vec[i].first]<<endl;
				}
			}
			vector<int> z;
			...
			Rep(i, n) if(del[i]==0 && play[i]==0) cout<<name[i]<<" advances with wildcard\n";
			cnt=(cnt+1)/2;
		}
		Rep(i, n) if(del[i]==0) cout<<"Winner: "<<name[i]<<endl<<endl;
	}
	return 0;
}
