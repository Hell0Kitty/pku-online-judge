...
void dp() {
  ...
  double ok31 =
      P31 /
      (1 - Pr1);  // 在第一个人不听重复“未命中->抢球成功"过程后投三分并命中
                  // (1+Pr1+...+Pr1^n) * P31 = P31 / (1-Pr1)
  double ok21 =
      P21 /
      (1 - Pr1);  // 在第一个人不听重复“未命中->抢球成功"过程后投两分并命中
  double okf1 =
      Pf1 / (1 - Pr1);  // 在第一个人不听重复“未命中->抢球成功"过程后抢球失败
  ...
  for (int i = n - 1; i >= 0; i--)
    for (int j = n - 1; j >= 0; j--) {
      ...
      f[i][j][0] = (trans1 + trans2 * okf1) /
                   (1 - okf1 * okf2);  // 两个人不停重复交换控球权概率 (1 +
                                       // okf1*okf2 + (okf1*okf2)^2 +....+
                                       // (okf1*okf2)^n) = 1 / (1 - okf1 * okf2)
