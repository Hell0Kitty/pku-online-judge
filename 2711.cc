  ...
  while (dis[st] <= en + 1) {
    ...
    if (!adv) {
      ...
      for (int e = fst[u]; ~e; e = nxt[e])
        if (cap[e] > flw[e] && dis[to[e]]) nw = min(nw, dis[to[e]]);
      if (!--num[dis[u]]) break;
      ++num[dis[u] = nw + 1];
      cur[u] = fst[u];
...
int main() {
  for (T = nxtInt(); T; --T) {
    ...
    while ((sc = getchar()) >= '0' && sc <= '9') {
      if (sc -= '0') map[0][c] = ++plr, ph[plr] = sc, px[plr] = 0, py[plr] = c;
      ++c;
    }
    for (int i = 1; i < r; ++i)
      for (int j = 0; j < c; ++j) {
        while ((sc = getchar()) < '0' || sc > '9')
          ;
        if (sc -= '0')
          map[i][j] = ++plr, ph[plr] = sc, px[plr] = i, py[plr] = j;
      }
    ...
    for (int i = 0; i < r; ++i)
      for (int j = 0; j < c; ++j) {
        while ((sc = getchar()) ^ '.' && sc ^ 'L')
          ;
        if (sc == 'L')
          ++L, add_edge(st, map[i][j], 1), add_edge(map[i][j], st, 0);
      }
    printf("Case #%d: ", ++kase);
    switch (L -= ISAP(st, en)) {
      case 0:
