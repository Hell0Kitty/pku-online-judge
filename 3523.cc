...
#define N 15
short h1[N][N][N][N][N][N], h2[N][N][N][N][N][N];

const int dx[5] = {0, 0, -1, 1, 0}, dy[5] = {-1, 1, 0, 0, 0};

inline short &h(short h[][N][N][N][N][N], const dat &cur) {
  return h[cur.pos[0].x - 1][cur.pos[0].y - 1][cur.pos[1].x - 1]
          [cur.pos[1].y - 1][cur.pos[2].x - 1][cur.pos[2].y - 1];
}
...
inline void expand(dat &cur, deque<dat> &que, short h1[][N][N][N][N][N]) {
  ...
  if (g == 1) {
    for (d1 = 0; d1 < 4; d1++) {
      p1 = cur.pos[0].x + dx[d1], q1 = cur.pos[0].y + dy[d1];
      if (Map[p1][q1] == ' ') {
        cur.pos[0] = PSS(p1, q1);
        cur.step++;
        if (h(h1, cur) < 0) {
    ...
    for (d1 = 0; d1 < 5; d1++) {
      ...
      for (d2 = 0; d2 < 5; d2++)
        if (d1 + d2 < 8) {
          p2 = cur.pos[1].x + dx[d2], q2 = cur.pos[1].y + dy[d2];
    ...
    for (d1 = 0; d1 < 5; d1++) {
      ...
      for (d2 = 0; d2 < 5; d2++) {
        ...
        if (p1 != cur.pos[1].x || q1 != cur.pos[1].y || p2 != cur.pos[0].x ||
            q2 != cur.pos[0].y)
          for (d3 = 0; d3 < 5; d3++)
            if (d1 + d2 + d3 < 12) {
              p3 = cur.pos[2].x + dx[d3], q3 = cur.pos[2].y + dy[d3];
              if (Map[p3][q3] != ' ') continue;
              if (p2 == p3 && q2 == q3 || p3 == p1 && q3 == q1) continue;
