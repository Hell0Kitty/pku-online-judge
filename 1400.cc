...
void infix(){
    ...
    char stack[len][len],s1[len],s2[len]; //s1,s2分别是参与运算的前面和后面的表达式，可以想想一下 M*K 这里的M和K为表达式 //stack下面有介绍
    ...
    for(i=p=0;i<n;i++){
        ...
        case '/':k=(post[i]=='*'||post[i]=='/')?2:1; // k记录优先级

            c=(post[i]=='-'||post[i]=='/')?true:false; //记录是否需要括号

            //判断是不是需要（） 设想一下: 表达式 ： m L h 这里的小写字母代表表达式，L代表运算符，如果m中有运算符，

            //那么m中的运算符的优先级必然大于或者等于L不然的话就要加（），例如:(a+b)*c，同理h中如果有运算符的话那么该运算符的优先级必然大于L，

            //或者当L的优先级和h中的运算符的优先级相同时L为‘+’或‘*’，不然也要加（），例如: a-(b+c) ; a/(b*c) ;
