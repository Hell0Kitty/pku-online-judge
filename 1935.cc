...
void DFS(int p,int pre){
    ...
    for(int l=head[p];l;l=e[l].nex){
		...
		if(vis[son] || dp[son][1]!=0){
			...
			ma=max(ma,dp[son][1]-dp[son][0]+e[l].w);//此处的dp[son][1]-dp[son][0]+e[l].w不能直接写成dp[son][0]+e[l].w
			                                         //因为要找以son为根的数中距离son最长的要访问的节点 dp[son][0]其实指的是访问完以son为根的子树中
			dp[p][1]+=(dp[son][1]+2*e[l].w);          //所有要访问的节点的最小路径 这样用dp[son][1]-dp[son][0]就表示访问完son为根的子树中所有要访问的节点
