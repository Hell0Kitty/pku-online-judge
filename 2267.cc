...
void add_Edge(int u, int v, int start, int end, int time){
    e[cnt].v = v, e[cnt].start = start, e[cnt].end = end, e[cnt].time = time, e[cnt].next = head[u];
    head[u] = cnt++;
}
struct point{
    int end, blood;
...
void spfa(int s, int t){
    ...
    while(!q.empty()){
        ...
        for(int i = head[u]; i + 1; i = e[i].next){
            int v = e[i].v;
            int start = e[i].start;
            if((tmp.end < 18 && start < 18 && tmp.end <= start) || (tmp.end >= 18 &&(start >= tmp.end || start < 18)))
...
int main(){
    ...
    while(tcase --){
        ...
        while(m --){
            ...
            start %= 24;
            if(!(start <= 6 && start + time <= 6 || start >= 18 && start + time <= 30))continue;
