...
void go(int n) {
  ...
  for (int i = 0; i < Q.size(); i++) {
    if (node[Q[i]].flag) {
      ans += "1";
      for (int j = 4 * Q[i] - 2; j < 4 * Q[i] + 2; j++) Q.push_back(j);
    } else {
      ans += "0";
      if (node[Q[i]].num)
        ans += "1";
  ...
  for (int i = 0; i < ans.length(); i += 4) {
    int tmp = 8 * (ans[i] - '0') + 4 * (ans[i + 1] - '0') +
              2 * (ans[i + 2] - '0') + 1 * (ans[i + 3] - '0');
    printf("%X", tmp);
  }
  printf("\n");
}
...
int main() {
  ...
  while (T--) {
    ...
    scanf("%d", &n);
    for (int i = 0; i < n; i++)
      for (int j = 0; j < n; j++) scanf("%d", &bin_map[i][j]);
    go(n);
  }
}
