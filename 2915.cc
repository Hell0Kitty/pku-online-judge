...
void inline Add_mid0(int idx, int cnt, int cost) {
  for (int i = 0; i < mid_sz[idx][0]; i++) {
    ...
    } else if (cost - cnt <= mid[idx][0][i].second - mid[idx][0][i].first) {
      swap(mid[idx][0][i], mid[idx][0][--mid_sz[idx][0]]);
      i--;
    }
  }
...
void inline Add_mid1(int idx, int cnt, int cost) {
  for (int i = 0; i < mid_sz[idx][1]; i++) {
    ...
    } else if (mid[idx][1][i].second >= cost) {
      swap(mid[idx][1][i], mid[idx][1][--mid_sz[idx][1]]);
      i--;
    }
  }
  ...
  while (scanf("%d", &m) != EOF) {
    ...
    for (int i = 1; i < n; i++) {
      ...
      for (int cj = ci - 1, j; cj >= 0; cj--) {
        ...
        for (int ck = ci, k; ck > cj; ck--) {
          ...
          for (int i_mid = 0; i_mid < mid_sz[ck][0]; i_mid++) {
            ...
            if (dp[j][j] > mid[ck][0][i_mid].first) {
              ...
              dp[j][i] =
                  min(dp[j][i], cost + dp[j][j] - mid[ck][0][i_mid].first);
            } else {
        ...
        for (int k = j - 1; k >= 0; k--)
          dp[k][i] = min(dp[k][i], dp[k][j - 1] + dp[j][i]);
      }
    }
    printf("%d\n", dp[0][n - 1]);
  }
