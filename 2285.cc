...
int Trans_num(int arr[3][3]) {
  int st = 0;
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) st |= arr[i][j] << (i * 5 + j);
  while (!(st & 31)) st >>= 5;
  while (!(st & 0x421)) st >>= 1;
  return st;
}
...
void Route() {
  memcpy(q, p, sizeof(q));
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) p[i][j] = q[2 - j][i];
}
...
void dfs(int cur, int pre, int pos, int add) {
  ...
  for (int i = pos; i < top; i++) {
    int stu = state[i];
    do {
      if ((pre & stu) == stu) dfs(cur, pre ^ stu, i, add + bv[i]);
      stu <<= 1;
    } while (!(stu & 0x421));
  }
}
...
int main() {
  while (~scanf("%d", &n) && n) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        scanf("%s", pic[j]);
        for (int k = 0; k < 3; k++) p[j][k] = (pic[j][k] == '#');
      }
      tmp = Trans_num(p);
      bv[tmp] = min(bv[tmp], w);
    ...
    for (int i = 1; i < MSK; i++)
      if (bv[i] < INF) state[top] = i, bv[top++] = bv[i];
    solve();
  }
  return 0;
}
