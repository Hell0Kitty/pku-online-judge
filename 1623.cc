...
string dfs(int len,int x,int y,int deep,int &temp)
{
    ...
    if(len==2)
    {
        s=dfs(len/2,x,y,deep+1,temp)+dfs(len/2,x,y+len/2,deep+1,temp)
            +dfs(len/2,x+len/2,y,deep+1,temp)+dfs(len/2,x+len/2,y+len/2,deep+1,temp);
        int l=s.length();
    ...
    else
    {
        ...
        if(flag0==4){temp=0;s="0000";}
        else if(flag1==4){temp=1;s="1111";}
