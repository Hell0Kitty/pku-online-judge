          ...
          for (int k = 0; k < 4; k++) {
            ...
            while (xx >= 0 && xx <= n + 1 && yy >= 0 && yy <= m + 1) {
              if (tile[xx][yy].alpha != '.') {
                ...
                if (tile[xx][yy].alpha == tile[sx][sy].alpha) V.PB(MP(xx, yy));
                break;
              } else
...
bool refresh() {
  ...
  for (int r = 1; r <= n; r++)
    for (int c = 1; c <= m; c++) tmp[r][c] = tile[r][c];
  bool rsh = false;
  for (int r = 1; r <= n; r++)
    for (int c = 1; c <= m; c++)
      if (tile[r][c].alpha != '.') {
  ...
  for (int r = 1; r <= n; r++)
    for (int c = 1; c <= m; c++) tile[r][c] = tmp[r][c];
  return rsh;
}
void solve() {
  while (true) {
    ...
    for (int ln = 1; ln <= 3 && !move; ln++) {
      for (int r = 1; r <= n && !move; r++)
        for (int c = 1; c <= m && !move; c++) {
          if (tile[r][c].alpha != '.' && try_to_remove(r, c, ln)) move = true;
        }
    }
    if (move)
      while (refresh())
...
int main() {
  ...
  while (~scanf("%d %d", &n, &m) && n + m) {
    ...
    printf("Case %d\n", ++T);
    for (int r = 1; r <= n; r++) {
      for (int c = 1; c <= m; c++) putchar(tile[r][c].alpha);
      puts("");
    }
  }
  return 0;
