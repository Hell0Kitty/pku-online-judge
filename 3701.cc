...
const int edg[12][2] = {10, 3,  13, 7,  5,  16, 1,  19, 46, 37, 48, 34,
                        50, 40, 52, 43, 23, 24, 26, 27, 29, 30, 32, 21};
const int vertex[8][3] = {0,  9,  20, 11, 12, 6,  8,  14, 15, 2,  17, 18,
                          45, 35, 36, 38, 39, 47, 53, 41, 42, 51, 33, 44};
...
void turn(int order) {
  ...
  for (int i = 0; i < 12; i++) a[List[x][i]] = b[List[x][(i + d + 12) % 12]];
  if (order % 6 < 2 || order % 6 > 3) {
    int x = (order / 6) * 2 + (order % 6) / 4, d = (order % 2) ? -2 : 2;
    for (int i = 0; i < 8; i++) a[Rota[x][i]] = b[Rota[x][(i + d + 8) % 8]];
  }
}
...
bool BUG() {
  ...
  for (int i = 0; i < 8; i++)
    v += a[vertex[i][0]] || a[vertex[i][1]] || a[vertex[i][2]];
  return c < 1 || e < 4 || v < 4;
}
