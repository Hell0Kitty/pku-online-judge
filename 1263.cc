...
Point pLinePoint(Point p,Line l)
{
	...
	p2.x=((l.b*l.b-l.a*l.a)*p.x-2*l.a*l.b*p.y-2*l.a*l.c)/d;
...
//求直线与圆的交点（保证有交点）
void interLineCircle(double r,Point center,Point l1,Point l2,Point &p1,Point &p2)
{
	Point p=center;
	p.x+=l1.y-l2.y;
	p.y+=l2.x-l1.x;
	...
	while(true)
	{
		...
		for(i=1;i<=n;i++)
			if(i!=node.cnt&&c[i].r-pToline(c[i].center,l)>eps)
			{
				Point p[2];
				double dis[2];
				interLineCircle(c[i].r,c[i].center,orign,temp,p[0],p[1]);//求直线与圆的交点
