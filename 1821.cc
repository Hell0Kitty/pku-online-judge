...
inline bool cmp(const RE &a, const RE &b) { return a.s < b.s; }

void read() {
  for (int i = 1; i <= k; i++) scanf("%d%d%d", &re[i].l, &re[i].p, &re[i].s);
...
void go() {
  ...
  for (int i = 1; i <= k; i++) {
    for (int j = 0; j < re[i].s; j++)
      dp[i][j] = dp[i - 1][j];  //第i个粉刷匠不刷任何墙
    int h = 0, t = 0;
    for (int j = l[i], tmp; j < re[i].s;
         j++)  //将dp[i-1]层的最优状态存入单调队列
    {
