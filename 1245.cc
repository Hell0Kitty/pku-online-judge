...
int main() {
  ...
  while (scanf("%d", &n)) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i > 0 && ranks[i].sol == ranks[i - 1].sol &&
          ranks[i].tot == ranks[i - 1].tot && ranks[i].g == ranks[i - 1].g) {
        ranks[i].ind = ranks[i - 1].ind;
      } else {
        ranks[i].ind = i + 1;
      }
      printf("%d %-10s %d %4d %3d %3d %3d %3d %3d %3d %3d %3d\n", ranks[i].ind,
             ranks[i].name, ranks[i].sol, ranks[i].tot, ranks[i].g,
             ranks[i].p[0], ranks[i].p[1], ranks[i].p[2], ranks[i].p[3],
             ranks[i].p[4], ranks[i].p[5], ranks[i].p[6]);
    }
  }
  return 0;
}
