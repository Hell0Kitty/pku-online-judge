...
typedef struct TPoint { double x, y; } TPoint;
...
typedef struct TLine { double a, b, c; } TLine;
...
double disPointToSeg(TPoint p1, TPoint p2, TPoint p3) {
  ...
  return fabs(l1.a * p1.x + l1.b * p1.y + l1.c) /
...
int main() {
  ...
  while (scanf("%d%d", &n, &m) && n) {
    ...
    if (polygonArea(polygon1) < 0)
      ChangeClockwise(polygon1);
    if (polygonArea(polygon2) < 0)
      ChangeClockwise(polygon2);
    ymin1 = inf, ymax2 = -inf;
    for (i = 0; i < n; i++)
      if (polygon1.p[i].y < ymin1)
        ymin1 = polygon1.p[i].y, k1 = i;
    for (i = 0; i < m; i++)
      if (polygon2.p[i].y > ymax2)
        ymax2 = polygon2.p[i].y, k2 = i;
    double SlewRate = 0;
    ...
    while (Slope <= 360) {
      ...
      if (fabs(angle1 - angle2) < eps) {
        d = disPallSeg(polygon1.p[k1], polygon1.p[(k1 + 1) % n], polygon2.p[k2],
                       polygon2.p[(k2 + 1) % m]);
        if (d < ans)
          ans = d;
        k1++;
        ...
        d = disPointToSeg(polygon2.p[k2], polygon1.p[k1],
                          polygon1.p[(k1 + 1) % n]);
        if (d < ans)
          ans = d;
        k1++;
        ...
        d = disPointToSeg(polygon1.p[k1], polygon2.p[k2],
                          polygon2.p[(k2 + 1) % m]);
        if (d < ans)
          ans = d;
        k2++;
