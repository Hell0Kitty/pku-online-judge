...
typedef struct { double x, y; } SPoint;
...
int sum[COL];
int idx, n, m;
int dir[18] = {0,   1,   2,    4,    8,    16,   32,    64,    128,
  ...
  for (i = NC[0]; i; i = NC[i]) {
    if (!sign[i]) {
      ...
      for (j = NR[i]; j != i; j = NR[j]) {
        for (k = NC[j]; k != j; k = NC[k]) sign[markc[k]] = 1;
      }
    }
  }
  return count;
...
int Dancing_links(int num) {
  ...
  for (i = NR[c]; i != c; i = NR[i]) {
    ...
    if (Dancing_links(num + 1)) return 1;
  ...
  while (T--) {
    ...
    while (low <= high) {
      ...
      for (i = l = 1; i <= n; i++) {
        bit = dir[i];
        for (j = i + 1; j <= n; j++) {
          if (Dis(pt[i], pt[j]) <= mid) bit |= dir[j];
        }
        if (hash[bit] != nr) {
          hash[bit] = nr, circle[l++] = bit;
        }
      }
