...
struct pp {
  ...
  bool operator<(const pp &temp) const {
    if (d + (long long)g == temp.d + (long long)temp.g) return num < temp.num;
  ...
  while (temp1 <= temp2) {
    for (i = temp1; i <= temp2; i++) {
      ...
      for (j = 0; j < rnum[queue[i][0]]; j++) {
        ...
        if ((queue[i][1] - v) % cc != 0) continue;
        value = ((queue[i][1] - v) % MOD + MOD) % MOD;
        int smin, smax;
        for (s = max(max(value - t, 0), value - cc + 1); s <= value; s++) {
          if (((queue[i][1] - v - s) % MOD + MOD) % MOD <= t &&
              dp[id][s] > dp[queue[i][0]][queue[i][1]] + v +
                              ((queue[i][1] - v - s) % MOD + MOD) % MOD) {
            dp[id][s] = dp[queue[i][0]][queue[i][1]] + v +
                        ((queue[i][1] - v - s) % MOD + MOD) % MOD;
            if (visit[id][s] == false) {
              visit[id][s] = true;
        ...
        for (s = max(max(value + 1, MOD + value - t), MOD + value - cc + 1);
             s < MOD; s++) {
          if (((queue[i][1] - v - s) % MOD + MOD) % MOD <= t &&
              dp[id][s] > dp[queue[i][0]][queue[i][1]] + v +
                              ((queue[i][1] - v - s) % MOD + MOD) % MOD) {
    ...
    for (i = 0; i < num[u.x]; i++) {
      ...
      for (j = jmin; j <= jmin + MOD / cc - 1; j++) {
        if (((j * cc - u.y) % MOD + MOD) % MOD <= t) {
          ...
          id = adj[0][u.x][i];
          nw.x = id;
          nw.d = u.d + (long long)adj[1][u.x][i] +
                 (long long)((j * cc - u.y) % MOD + MOD) % MOD;
          nw.y = nw.d % MOD;
          nw.g = dp[id][nw.y];
          if (nw.g < 1000000000) {
            nw.num = 1000000000;
            ...
            if (hash.size() == k) {
              it = hash.end();
              it--;
              if (it->first.d + it->first.g <= nw.d + nw.g) break;
            }
          }
        } else {
          int kp;
