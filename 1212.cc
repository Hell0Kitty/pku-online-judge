...
int Josephus(int n, int k) {
  ...
  if (n <= (int)vecJosephus.size()) return vecJosephus[n - 1];
...
int main(void) {
  ...
  for (int nSizeMin, nSizeMax; cin >> nSizeMin >> nSizeMax;) {
    if (nSizeMin > nSizeMax) swap(nSizeMin, nSizeMax);
    if (nSizeMin == 0 && nSizeMax == 0) break;
    vector<int> vecCover(nSizeMin / 2, 0);
    for (int nSize = nSizeMin, nJump = 15; nSize <= nSizeMax; ++nSize) {
      int j = Josephus(nSize, nJump);
      if (j > nSize / 2) j = nSize - j;
      if (j < vecCover.size()) vecCover[j] = 1;
    }
