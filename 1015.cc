...
void solve_case() {
  ...
  int sol[JURY_MAX];
  for (i = 0; i < m; i++)  //初始化
    for (j = 0; j < 2 * VALUE_MAX * m + 1; j++) last[i][j] = sum[i][j] = -1;

  for (i = 0; i < n; i++)  //初始取一个组合的结果
  ...
  for (i = 0; i <= VALUE_MAX * m; i++)  //寻找最小的情况
    if (sum[m - 1][VALUE_MAX * m + i] >= 0 ||
        sum[m - 1][VALUE_MAX * m - i] >= 0) {
  ...
  printf("Best jury has value %d for prosecution and value %d for defence:\n",
         (sum[m - 1][i] + (i - VALUE_MAX * m)) / 2,
         (sum[m - 1][i] - (i - VALUE_MAX * m)) / 2);
  for (j = m - 1; j >= 0; j--)  //依次递推。
  {
    sol[j] = last[j][i] + 1;
