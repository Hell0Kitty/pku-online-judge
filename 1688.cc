...
void intersection_circle_circle(pt c1, double r1, pt c2, double r2, pt &p1,
                                pt &p2) {
  pt u, v;
...
int solve() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < cut[i].size(); j++) {
      if (ccnt[i] == 0) tAng += toAng(lastAng, cut[i][j].vAng) + cut[i][j].rAng;
      ccnt[i] += cut[i][j].del;
      if (ccnt[i] == 0) lastAng = cut[i][j].vAng;
    }
  }
