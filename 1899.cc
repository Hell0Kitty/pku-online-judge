...
bool IntersectRect(LPRECT lpRect1, LPRECT lpRect2) {
  if (((lpRect1->left < lpRect2->left) ? (lpRect1->right < lpRect2->left)
                                       : (lpRect2->right < lpRect1->left)) ||
      ((lpRect1->top < lpRect2->top) ? (lpRect1->bottom < lpRect2->top)
                                     : (lpRect2->bottom < lpRect1->top)))
    return false;
...
int main(int argc, char* argv[]) {
  ...
  for (LPRECT pRect = dqDstRect.begin(); pRect != dqDstRect.end(); pRect++) {
