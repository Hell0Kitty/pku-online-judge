...
void solveEqua(char* str, int l, int r, int& a, int& b, int& c) {
  ...
  for (int i = l; i < r; i++) {
    ...
    bool first = 1;
    while (str[i] != ' ' && i < r) {
      if (issign(str[i]) && str[i + 1] == ' ') op = str[i] == '+' ? 1 : -1;
      ...
      if (isdigit(str[i]))  // term
      {
        ...
        if (i + 1 == r || str[i + 1] == ' ') c += val * sign * op;
      }
...
void solve(int a1, int b1, int c1, int a2, int b2, int c2, int& P, int& Q) {
  ...
  if ((!a1 && !b1 && a2 && b2) || (a1 && b1 && !a2 && !b2)) return;
  if (!a1 && !a2 && b1 && b2) {
    if (c1 * b2 != c2 * b1)
      return;
    else {
...
int main() {
  ...
  while (t--) {
    ...
    int P = 0, Q = 0;
    solve(b1 - b2, a2 - a1, c2 - c1, b3 - b4, a4 - a3, c4 - c3, P, Q);
    printAns(P, Q);

    P = Q = 0;
    solve(a1 - a2, b2 - b1, c2 - c1, a3 - a4, b4 - b3, c4 - c3, P, Q);
    printAns(P, Q);
    if (t != 0) puts("");
    getchar();
  }
