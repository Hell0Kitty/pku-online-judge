...
int getanc(int a) {
  if (father[a] != a) return father[a] = getanc(father[a]);
  return a;
}
...
void makeareas() {
  ...
  for (i = 0; i < namecount; i++) father[i] = i;
  sort(edges, edges + n);
  for (i = 0; i < n; i++) {
    if (edges[i].v1 == 0 || edges[i].v2 == 0) continue;
...
void calculate() {
  ...
  for (i = 0; i < n; i++)
    if (used[edges[i].v1][edges[i].v2]) answer += edges[i].w;
  fans = answer;
}
