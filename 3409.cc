...
{
	int i,id1;
	for(i=head[key];i!=-1;i=edges[i].next) if(edges[i].id!=id&&!visit[edges[i].id])
	{
		id1=edges[i].id;
...
{
	int i;
	for(i=1;i<=num;i++) if(ok(a[i].x,x)&&ok(a[i].y,y))
	{
		a[i].num++;
		return;
	}
...
int isclosed()
{
	...
	for(i=0;i<mod;i++)
	{
		...
		{
			id=edges[j].id;
			if(edges[j].flag==0) Add(p[id].x1,p[id].y1);
			else Add(p[id].x2,p[id].y2);
		}
		for(j=1;j<=num;j++) if(a[j].num&1) return 0;
	}
	return 1;
}
