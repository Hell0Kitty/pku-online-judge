...
struct rec {
  ...
  bool operator<(const rec &temp) const {
    if (dx1 == temp.dx1) return dy1 < temp.dy1;
    return dx1 < temp.dx1;
  }
};
...
struct segment {
  int va1, va2;
  bool operator<(const segment &temp) const { return va1 < temp.va1; }
};
...
void heapify(int id) {
  ...
  if (l < size && heap[l] < heap[ii]) ii = l;
  if (r < size && heap[r] < heap[ii]) ii = r;
  if (ii != id) {
...
void add() {
  ...
  for (j = id1 + 1; j <= id2 - 1; j++) {
    for (s = 0; s < cont[0][j]; s++) {
      if (seg[0][j][s].va1 <= heap[size].y1 &&
          seg[0][j][s].va2 >= heap[size].y2) {
  ...
  for (j = id1 + 1; j <= id2 - 1; j++) {
    for (s = 0; s < cont[1][j]; s++) {
      if (seg[1][j][s].va1 <= heap[size].x1 &&
          seg[1][j][s].va2 >= heap[size].x2) {
...
void run() {
  ...
  add();
  while (size) {
    if (heap[0].dx1 == 1000000000) break;
...
int main() {
  ...
  while (scanf("%d", &n) && n) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (a[i].x1 < in[0]) in[0] = a[i].x1;
      if (a[i].x2 > ax[0]) ax[0] = a[i].x2;
      if (a[i].y1 < in[1]) in[1] = a[i].y1;
      if (a[i].y2 > ax[1]) ax[1] = a[i].y2;
      list[0][num[0]++] = a[i].x1;
      list[0][num[0]++] = a[i].x2;
      list[1][num[1]++] = a[i].y1;
      list[1][num[1]++] = a[i].y2;
    ...
    for (i = 0; i < 2; i++) {
      ...
      cnt[i] = 1;
      for (j = 1; j < num[i]; j++) {
        if (list[i][j] != list[i][cnt[i] - 1]) list[i][cnt[i]++] = list[i][j];
      }
    }
    ...
    for (i = 0; i < 2; i++) {
      for (j = 0; j < cnt[i]; j++) {
        ...
        for (s = 1; s < cont[i][j]; s++) {
          ...
          if (max < seg[i][j][s].va2) max = seg[i][j][s].va2;
        }
        seg[i][j][ncnt[i][j]++].va2 = max;
        cont[i][j] = ncnt[i][j];
      }
