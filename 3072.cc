...
struct node {
  double angle, dist;
  int num, pre;
  node(int n, int p, double a, double d) : num(n), pre(p), angle(a), dist(d) {}
  node() {}
...
struct nxtPnt {
  int nxt;
  double dist, angle;
  nxtPnt(int n, double a, double d) : nxt(n), angle(a), dist(d) {}
  nxtPnt() {}
};
...
void searchAns(int n) {
  ...
  while (!que.empty()) {
    node top = que.top();
    que.pop();
    //	cout<<"debug:"<<top.num<<' '<<top.dist<<' '<<top.pre<<'
    //'<<vis[top.num][top.pre]<<endl;
    int cur = top.num;
    int pre = top.pre;
...
int main() {
  ...
  while (scanf("%d%d", &r, &n) != EOF) {
    ...
    initAng = atan2((double)(y[n - 1] - y[0]), (double)(x[n - 1] - x[0])) / pi *
              180.0;
    for (int i = 0; i < n; i++) vec[i].clear();
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        double distBet = sqrt((double)(x[i] - x[j]) * (x[i] - x[j]) +
                              (double)(y[i] - y[j]) * (y[i] - y[j]));
        if (distBet <= r) {
          double angleBet1 =
              atan2((double)(y[j] - y[i]), (double)(x[j] - x[i]));
          double angleBet2 =
              atan2((double)(y[i] - y[j]), (double)(x[i] - x[j]));
