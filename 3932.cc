...
void move_to_other(int deep, bool had, int* n, int op, int pos, int s,
                   int preop, int one = 0);
bool unchange_add(int pos, int s, int preop, int one = 0);
void move_to_orgin(int deep, int op, int* n, int pos, int s, int preop,
                   int one = 0);
...
void move_to_orgin(int deep, int op, int* n, int pos, int s, int preop,
                   int one /* = 0*/) {  // op 1有移入matches -1,移走 matches
  if (deep == 3) {
    if (0x03 != op) return;
...
void print_ans(int mn) {
  ...
  printf("%s + %s = %s\n", ans_num[0].c_str(), ans_num[1].c_str(),
         ans_num[2].c_str());
  printf("*****************\n");
}
