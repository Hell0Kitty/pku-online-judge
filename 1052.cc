...
void makesur() {
  ...
  for (p = 0; p < 3; p++) {
    for (k = 2; k <= 4; k++)
      for (i = 1; i <= n; i++)
        for (j = 1; j <= n; j++) sur[p][k][j][n - i + 1] = sur[p][k - 1][i][j];

    for (k = 1; k <= 4; k++)
      for (i = 1; i <= n; i++)
        for (j = 1; j <= n; j++) sur[p][k + 4][n - i + 1][j] = sur[p][k][i][j];
  }
}
...
void fill(int a1, int a2, int a3) {
  ...
  for (i = 1; i <= n; i++)
    for (j = 1; j <= n; j++)
      if (!sur[0][a1][i][j])
        for (k = 1; k <= n; k++) cube[i][j][k] = 0;
  for (i = 1; i <= n; i++)
    for (j = 1; j <= n; j++)
      if (!sur[1][a2][i][j])
        for (k = 1; k <= n; k++) cube[k][i][j] = 0;
  for (i = 1; i <= n; i++)
    for (j = 1; j <= n; j++)
      if (!sur[2][a3][i][j])
        for (k = 1; k <= n; k++) cube[i][k][j] = 0;
}
