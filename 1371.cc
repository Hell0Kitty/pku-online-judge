...
struct pt {
    ...
    pt trunc(int s) const { int l = len(); return pt(x*s/l, y*s/l); }
            ...
            if (dfn[u] == 0) 
            {
                ...
                if (low[u] >= dfn[v])	
                {
                    ...
                    {			
                        ...
                        top--;
                    }while (!(stack[top+1].first == v && stack[top+1].second == u));
                    if (v != 1) ge[v] = true;			
                }	
                if (v == 1)
                    if (++son > 1) ge[v] = true; 
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < n; ++j) {
                ...
                if (i != j) {
                        ...
                        pt c = cross_point(seg[i].first, seg[i].second, seg[j].first, seg[j].second);
