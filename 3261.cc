...
#include <stdlib.h>
#define MAXD 20010
int N, K, r[MAXD], sa[MAXD], rank[MAXD], height[MAXD], wa[MAXD], wb[MAXD],
    ws[MAXD], wv[MAXD];
void init() {
  int i, j, k;
  for (i = 0; i < N; i++) {
...
void da(int n, int m) {
  ...
  for (i = 0; i < n; i++) ws[i] = i, x[i] = r[i];
  qsort(ws, n, sizeof(ws[0]), cmp1);
  for (i = 0; i < n; i++) sa[i] = ws[i];
  for (p = 1, j = 1; p < n; j *= 2, m = p) {
    ...
    if (m > 1000000) {
      for (i = 0; i < n; i++) ws[i] = i;
      qsort(ws, n, sizeof(ws[0]), cmp2);
      for (i = 0; i < n; i++) sa[i] = y[ws[i]];
    } else {
