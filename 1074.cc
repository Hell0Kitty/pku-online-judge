...
int command(int machine, char Command[][65], int List, int Number) {
  for (int i = 0; i < List; ++i) {
    ...
    Word[0].Start = 0;
    Word[0].End = space1 - 1;
    Word[1].Start = space1 + 2;
    Word[1].End = space2 - 1;
    Word[2].Start = space2 + 1;
    Word[2].End = space3 - 1;
...
void calculate(double* Var, asmmble Asm) {
  switch (Asm.Operator) {
    case ':':
      Var[Asm.Varible1] = Var[Asm.Varible2];
      break;
    case '+':
      Var[Asm.Varible1] += Var[Asm.Varible2];
      break;
...
void dp(int number1, int number2, int count) {
  ...
  for (int i = 1; i <= number1; ++i) {
    P[i][0] = 0.5 * P[i - 1][0];
    for (int k = 0; k < count; ++k) T[i][0][k] = T[i - 1][0][k];
    calculate(T[i][0], Asmmble[0][i]);
  }

  for (int j = 1; j <= number2; ++j) {
    P[0][j] = 0.5 * P[0][j - 1];
    for (int k = 0; k < count; ++k) T[0][j][k] = T[0][j - 1][k];
  ...
  for (int k = 0; k < count; ++k) Varible[k].Value = T[number1][number2][k];

  qsort(&Varible[4], count - 4, sizeof(Varible[0]), cmp);

  int start = 4;
  while (Varible[start].IsConst) ++start;
  for (int i = start; i < count; ++i) printf("%.4lf\n", Varible[i].Value);
}
