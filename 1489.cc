...
int calcul(char s[], int cnt[])  //将字符串记录成数组，返回十进制的值
{
  ...
  while (i < len) {
    while (i < len && s[i] == '|') cnt[0]++, i++;
    while (i < len && s[i] == 'n') cnt[1]++, i++;
    while (i < len && s[i] == '9') cnt[2]++, i++;
    while (i < len && s[i] == '8') cnt[3]++, i++;
    while (i < len && s[i] == 'r') cnt[4]++, i++;
    i++;
  }
  for (i = 4; i >= 0; i--) num = num * 10 + cnt[i];
  return num;
}
...
void slove() {
  ...
  while (lessthan(ccnt, bcnt)) {
    ...
    for (; len < 34; len++) printf(" ");
    len = Print(acnt);
    printf("\n");
    if (f)
      for (i = 0; i <= 4; i++) dcnt[i] += acnt[i];
