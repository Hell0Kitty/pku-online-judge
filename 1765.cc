...
bool cmp(node a, node b) { return a.x1 < b.x1; }
...
int main() {
  ...
  while (scanf("%d", &n) != EOF) {
    pointcnt = 0;
    for (int i = 0; i < n; i++) {
      scanf("%d%d%d%d", &segment[i].x1, &segment[i].y1, &segment[i].x2,
            &segment[i].y2);
      segment[i].num = i;
      segment[i].next = -1;
      segment[i].w = 0;
      segment[i].deg = 0;
      segment[i].k = (1.0 * segment[i].y2 - segment[i].y1) /
    ...
    for (int i = 0; i < pointcnt; last = now, i++) {
      ...
      while (segment[tail].x1 == now && tail < n) {
        ...
        for (int j = 0; j < levelcnt; j++) {
          if ((segment[a[j]].y1 + segment[a[j]].k * (now - segment[a[j]].x1)) <
              segment[tail].y1)
            k = j;
        }
      ...
      for (int j = 1; j < levelcnt; j++) {
        if ((segment[a[j]].x1 == now && segment[a[j]].y1 < segment[a[j]].y2) ||
            (segment[a[j]].x2 == now && segment[a[j]].y1 > segment[a[j]].y2)) {
