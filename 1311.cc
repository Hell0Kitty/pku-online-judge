...
int try_height(int i, int sw) {
  double dh = sw / r[i];
  int h = dh + .0000005;
  if (fabs(h - dh) < 0.000001) return h;
  return -1;
}
...
int try_weight(int i, int sh) {
  double dw = sh * r[i];
  int w = dw + .0000005;
  if (fabs(w - dw) < 0.000001) return w;
  return -1;
}
...
int fit(int w, int i, int j) {
  ...
  if (is_integer(h * r[i]) && is_integer(h * r[j])) return ret;
  return 0;
}
