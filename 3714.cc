...
double minL(double l1, double l2) { return l1 <= l2 ? l1 : l2; }
bool cmpy(int a, int b) { return m[a].y < m[b].y; }
...
double getMinLen(Point *s, int left, int right) {
  ...
  for (i = left; i <= right; ++i) {
    if (fabs(s[i].x - s[mid].x) <= rs) tmp[num++] = i;
  }
  sort(tmp, tmp + num, cmpy);
  double d = INF;
  for (i = 0; i < num; ++i) {
    for (j = i + 1; j < num; ++j) {
      if (fabs(s[tmp[i]].y - s[tmp[j]].y) >= rs) break;
      if (s[tmp[i]].flag != s[tmp[j]].flag &&
          (d = dis(s[tmp[i]], s[tmp[j]])) < rs)
        rs = d;
    }
  }
