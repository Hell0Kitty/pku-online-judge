...
int graham(Point pp[], Point res[]) {
  ...
  res[2] = pp[2];
  for (i = 2; i < 4; i++) {
    while (top && x_mult(pp[i], res[top], res[top - 1]) > -EP) top--;
    res[++top] = pp[i];
  }
  len1 = top;
  res[++top] = pp[2];
  for (i = 1; i >= 0; i--) {
    while (top != len1 && x_mult(pp[i], res[top], res[top - 1]) > -EP) top--;
    res[++top] = pp[i];
  }
  return top;
}
...
int main() {
  while (1) {
    ...
    if (sum == 4) {
      ...
      for (i = 0; i < 4; i++) ans += dist(p[i], c);
    } else {
      ans = INF;
      for (i = 0; i < 4; i++) {
        s = 0;
        for (j = 0; j < 4; j++) s += dist(p[i], p[j]);
        if (s < ans) ans = s;
      }
    }
    printf("%.4f\n", ans);
