...
int main() {
  int INF = 99999999, N, K, d[30][30], i, j, k, x, y, z, dp[256][30], e[8],
      v[30], c, b;
  string s, t;
  while (cin >> N >> K && N) {
    ...
    if (K)
      while (cin >> s >> t >> z, x = cityMap[s], y = cityMap[t],
    ...
    for (i = 1; i < 256; i++) {
      ...
      for (k = 0; k < N; k++) {
        ...
        v[k] = 0;
        for (j = 1; j < i; j++)
          if ((i | j) == i) dp[i][k] = min(dp[i][k], dp[j][k] + dp[i - j][k]);
      }
      // step2
      for (j = 0; b = INF, j < N; j++) {
        for (k = 0; k < N; k++)
          if (dp[i][k] <= b && !v[k]) b = dp[i][c = k];
        for (k = 0, v[c] = 1; k < N; k++)
