...
} grid[MAX_COORDINATE][MAX_COORDINATE];  // using down-left coner point to
                                         // present a gird
...
int work() {
  if (nemo.x < 0 || nemo.x >= EDGE || nemo.y < 0 || nemo.y >= EDGE) return 0;
  ...
  while (!pq.empty()) {
    Point u = pq.top();
    if (u.x == nemo.x && u.y == nemo.y) return u.door_cnt;
    pq.pop();
...
int main() {
  while (scanf("%d%d", &wall_num, &door_num),
         !(wall_num == -1 && door_num == -1)) {
    input();
    printf("%d\n", work());
  }
  return 0;
