...
void rotate(struct vector *x0, struct vector *y0, struct vector *z0, float a,
            int b) {
  struct vector x, y, z;
...
#define skal(X2, X1, Z) \
  ((X2->x - X1->x) * Z->x + (X2->y - X1->y) * Z->y + (X2->z - X1->z) * Z->z)
int intersect(struct vector *x1, struct vector *x2, struct vector *y1,
              struct vector *y2) {
  struct vector w;
  float X, Y, W, A, B, C, D, E;
  if ((fabs(x1->x - x2->x) < 0.000001) && (fabs(x1->y - x2->y) < 0.000001) &&
      (fabs(x1->z - x2->z) < 0.000001))
    return 0;
  if ((fabs(y1->x - y2->x) < 0.000001) && (fabs(y1->y - y2->y) < 0.000001) &&
      (fabs(y1->z - y2->z) < 0.000001))
    return 0;
  ...
  if (W < 0.000001) {
    ...
    W = troj(x1, x2, y1);
    W = W * 2 /
        sqrt(sqr(x2->x - x1->x) + sqr(x2->y - x1->y) + sqr(x2->z - x1->z));
    if (W > 0.001) return 0;
  } else {
    if ((fabs(x2->x - y1->x) < 0.000001) && (fabs(x2->y - y1->y) < 0.000001) &&
        (fabs(x2->z - y1->z) < 0.000001))
      return 0;
    w.x /= W;
    w.y /= W;
    w.z /= W;
