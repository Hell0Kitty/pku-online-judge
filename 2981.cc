...
bool dfs() {
  ...
  for (int fp = 0; fp < 2; fp++) {
    ...
    if (dfs()) return true;
    ans[iden] = -1;
    for (i = 0; i < hp.size(); i++) saf[hp[i]] = false;
    for (i = 0; i < num_p[flag][iden]; i++) {
      id = pos[flag][iden][i];
      for (j = 0; j < cl[id].num_sub; j++) {
        ip = cl[id].sc[j].id;
...
int main() {
  ...
  if (!dfs()) puts("mission impossible");
  for (i = 0; i < num_kind; i++) {
    if (ans[i] > 0)
      puts(kind[i].str);
    else {
