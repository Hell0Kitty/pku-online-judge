...
char sign(double a) { return a > 0 ? '-' : '+'; }
char sign2(double a) { return a > 0 ? '+' : '-'; }
...
int main() {
  ...
  while (std::cin >> a.x >> a.y >> b.x >> b.y >> c.x >> c.y) {
    ...
    printf("(x %c %.3f)^2 + (y %c %.3f)^2 = %.3f^2\n", sign(center.x),
           fabs(center.x), sign(center.y), fabs(center.y), r);
    printf("x^2 + y^2 %c %.3fx %c %.3fy %c %.3f = 0\n", sign(center.x),
           2 * fabs(center.x), sign(center.y), 2 * fabs(center.y),
           sign2(center.x * center.x + center.y * center.y - r * r),
           fabs(center.x * center.x + center.y * center.y - r * r));
    puts("");
  }
}
