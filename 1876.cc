...
int main() {
  ...
  while (true) {
    ...
    for (int i = 0; i < mail.recer.size(); i++) processed[i] = false;
    pfn = mail.recer.size();
    while (pfn) {
      int reci = findrec(processed, mail.recer.size());
      cout << "Connection between " << getmta(mail.sender) << " and "
           << getmta(mail.recer[reci]) << endl;
      kg();
      ...
      for (int i = reci; i < mail.recer.size(); i++) {
        if (getmta(mail.recer[i]) == recMtaStr) {
          ...
          pfn--;
          processed[i] = true;
          for (vector<MTA>::iterator p = allMta.begin(); p != allMta.end();
               p++) {
            if (p->name == recMtaStr) {
              finded = false;
